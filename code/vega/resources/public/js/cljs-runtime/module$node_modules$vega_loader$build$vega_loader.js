...
url=opt.href;return opt.localFile?$jscomp$async$this.file(url):$jscomp$async$this.http(url,options)})}function sanitize(uri,options){const $jscomp$async$this=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){options=vegaUtil.extend({},$jscomp$async$this.options,options);var fileAccess=$jscomp$async$this.fileAccess;const result={href:null};var isFile=allowed_re.test(uri.replace(whitespace_re,""));null!=uri&&"string"===typeof uri&&isFile||vegaUtil.error("Sanitize failure, invalid URI: "+
vegaUtil.stringValue(uri));const hasProtocol=protocol_re.test(uri);(isFile=options.baseURL)&&!hasProtocol&&(uri.startsWith("/")||"/"===isFile[isFile.length-1]||(uri="/"+uri),uri=isFile+uri);fileAccess=(isFile=uri.startsWith("file://"))||"file"===options.mode||"http"!==options.mode&&!hasProtocol&&fileAccess;isFile?uri=uri.slice(7):uri.startsWith("//")&&("file"===options.defaultProtocol?(uri=uri.slice(2),fileAccess=!0):uri=(options.defaultProtocol||"http")+":"+uri);Object.defineProperty(result,"localFile",
{value:!!fileAccess});result.href=uri;options.target&&(result.target=options.target+"");options.rel&&(result.rel=options.rel+"");"image"===options.context&&options.crossOrigin&&(result.crossOrigin=options.crossOrigin+"");return result})}function fileLoader(fs){return fs?filename=>new Promise((accept,reject)=>{fs.readFile(filename,(error,data)=>{error?reject(error):accept(data)})}):fileReject}function fileReject(){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){vegaUtil.error("No file system access.")})}
...
{})}function delimitedFormat(delimiter){const parse=function(data,format){const delim={delimiter};return dsv(data,format?vegaUtil.extend(format,delim):delim)};parse.responseType="text";return parse}function dsv(data,format){format.header&&(data=format.header.map(vegaUtil.stringValue).join(format.delimiter)+"\n"+data);return d3Dsv.dsvFormat(format.delimiter).parse(data+"")}function json(data,format){format=format&&format.property?vegaUtil.field(format.property):vegaUtil.identity;var JSCompiler_temp;
if(JSCompiler_temp=vegaUtil.isObject(data))JSCompiler_temp=!("function"===typeof Buffer&&vegaUtil.isFunction(Buffer.isBuffer)&&Buffer.isBuffer(data));return JSCompiler_temp?format(data):format(JSON.parse(data))}function topojson(data,format){let method,object,property,filter;data=json(data,format);format&&format.feature?(method=topojsonClient.feature,property=format.feature):format&&format.mesh?(method=topojsonClient.mesh,property=format.mesh,filter=filters[format.filter]):vegaUtil.error("Missing TopoJSON feature or mesh parameter.");
return(object=(object=data.objects[property])?method(data,object,filter):vegaUtil.error("Invalid TopoJSON object: "+property))&&object.features||[object]}function formats(name,reader){return 1<arguments.length?(format$jscomp$0[name]=reader,this):vegaUtil.hasOwnProperty(format$jscomp$0,name)?format$jscomp$0[name]:null}function parse$jscomp$0(data,types,timeParser,utcParser){if(data.length){var locale=vegaFormat.timeFormatDefaultLocale();timeParser=timeParser||locale.timeParse;utcParser=utcParser||
locale.utcParse;locale=data.columns||Object.keys(data[0]);var j,m;"auto"===types&&(types=inferTypes(data,locale));locale=Object.keys(types);var parsers=locale.map(field=>{var type=types[field];if(type&&(type.startsWith("date:")||type.startsWith("utc:"))){field=type.split(/:(.+)?/,2);type=field[1];if("'"===type[0]&&"'"===type[type.length-1]||'"'===type[0]&&'"'===type[type.length-1])type=type.slice(1,-1);return("utc"===field[0]?utcParser:timeParser)(type)}if(!typeParsers[type])throw Error("Illegal format pattern: "+
field+":"+type);return typeParsers[type]});var i=0;var n=data.length;for(m=locale.length;i<n;++i){var datum=data[i];for(j=0;j<m;++j){var field$jscomp$0=locale[j];datum[field$jscomp$0]=parsers[j](datum[field$jscomp$0])}}}}const protocol_re=/^([A-Za-z]+:)?\/\//,allowed_re=/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|cid|xmpp|file|data):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i,whitespace_re=/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205f\u3000]/g,isNumber=_=>!Number.isNaN(+_)&&!(_ instanceof Date),typeParsers=
{boolean:vegaUtil.toBoolean,integer:vegaUtil.toNumber,number:vegaUtil.toNumber,date:vegaUtil.toDate,string:vegaUtil.toString,unknown:vegaUtil.identity},typeTests=[_=>"true"===_||"false"===_||!0===_||!1===_,_=>isNumber(_)&&Number.isInteger(+_),isNumber,_=>!Number.isNaN(Date.parse(_))],typeList=["boolean","integer","number","date"];dsv.responseType="text";json.responseType="json";const filters={interior:(a,b)=>a!==b,exterior:(a,b)=>a===b};topojson.responseType="json";const format$jscomp$0={dsv,csv:delimitedFormat(","),
tsv:delimitedFormat("\t"),json,topojson};var loader=function(fetch,fs){return options=>({options:options||{},sanitize,load,fileAccess:!!fs,file:fileLoader(fs),http:httpLoader(fetch)})}("undefined"!==typeof fetch&&fetch,null);exports.format=format$jscomp$0;exports.formats=formats;exports.inferType=inferType;exports.inferTypes=inferTypes;exports.loader=loader;exports.read=function(data,schema,timeParser,utcParser){schema=schema||{};const reader=formats(schema.type||"json");reader||vegaUtil.error("Unknown data format type: "+
