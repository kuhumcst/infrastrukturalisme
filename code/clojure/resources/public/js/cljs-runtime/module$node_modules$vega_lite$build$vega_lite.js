...
case "color":case "fill":case "stroke":return"flexible"}}function binToString(bin){isBoolean(bin)&&(bin=normalizeBin(bin,void 0));return"bin"+keys$jscomp$0(bin).map(p=>isSelectionExtent(bin[p])?varName("_".concat(p,"_").concat(entries(bin[p]))):varName("_".concat(p,"_").concat(bin[p]))).join("")}function isBinning(bin){return!0===bin||isObject(bin)&&!bin.binned}function isBinned(bin){return"binned"===bin||isObject(bin)&&!0===bin.binned}function isSelectionExtent(extent){return null===extent||void 0===
extent?void 0:extent.selection}function autoMaxBins(channel){switch(channel){case "row":case "column":case "size":case "color":case "fill":case "stroke":case "strokeWidth":case "opacity":case "fillOpacity":case "strokeOpacity":case "shape":return 6;case "strokeDash":return 4;default:return 10}}function _defineProperty(obj,key,value){key in obj?Object.defineProperty(obj,key,{value,enumerable:!0,configurable:!0,writable:!0}):obj[key]=value;return obj}function _classPrivateFieldSet(receiver,privateMap,
value){privateMap=privateMap.get(receiver);if(!privateMap)throw new TypeError("attempted to set private field on non-instance");if(privateMap.set)privateMap.set.call(receiver,value);else{if(!privateMap.writable)throw new TypeError("attempted to set read only private field");privateMap.value=value}return value}function invalidSpec(spec){return"Invalid specification ".concat(JSON.stringify(spec),'. Make sure the specification includes at least one of the following properties: "mark", "layer", "facet", "hconcat", "vconcat", "concat", or "repeat".')}
function containerSizeNonSingle(name){return"".concat("width"==name?"Width":"Height",' "container" only works for single views and layered views.')}function containerSizeNotCompatibleWithAutosize(name){const fitDirection="width"==name?"x":"y";return"".concat("width"==name?"Width":"Height",' "container" only works well with autosize "fit" or "fit-').concat(fitDirection,'".')}function droppingFit(channel){return channel?'Dropping "fit-'.concat(channel,'" because spec has discrete ').concat(getSizeChannel(channel),
"."):'Dropping "fit" because spec has discrete size.'}function unknownField(channel){return"Unknown field for ".concat(channel,". Cannot calculate view size.")}function cannotProjectOnChannelWithoutField(channel){return'Cannot project a selection on encoding channel "'.concat(channel,'", which has no field.')}function cannotProjectAggregate(channel,aggregate){return'Cannot project a selection on encoding channel "'.concat(channel,'" as it uses an aggregate function ("').concat(aggregate,'").')}function selectionNotSupported(mark){return"Selection not supported for ".concat(mark,
" yet.")}function columnsNotSupportByRowCol(type){return'The "columns" property cannot be used when "'.concat(type,'" has nested row/column.')}function differentParse(field,local,ancestor){return'An ancestor parsed field "'.concat(field,'" as ').concat(ancestor," but a child wants to parse the field as ").concat(local,".")}function customFormatTypeNotAllowed(channel){return"Config.customFormatTypes is not true, thus custom format type and format for channel ".concat(channel," are dropped.")}function droppingColor(type,
opt){const {fill,stroke}=opt;return"Dropping color ".concat(type," as the plot also has ").concat(fill&&stroke?"fill and stroke":fill?"fill":"stroke",".")}function emptyFieldDef(fieldDef,channel){return"Dropping ".concat(stringify(fieldDef),' from channel "').concat(channel,'" since it does not contain any data field, datum, value, or signal.')}function incompatibleChannel(channel,markOrFacet,when){return"".concat(channel,' dropped as it is incompatible with "').concat(markOrFacet,'"').concat(when?
" when ".concat(when):"",".")}function invalidEncodingChannel(channel){return"".concat(channel,"-encoding is dropped as ").concat(channel," is not a valid encoding channel.")}function discreteChannelCannotEncode(channel,type){return'Using discrete channel "'.concat(channel,'" to encode "').concat(type,'" field can be misleading as it does not encode ').concat("ordinal"===type?"order":"magnitude",".")}function scalePropertyNotWorkWithScaleType(scaleType,propName,channel){return"".concat(channel,"-scale's \"").concat(propName,
'" is dropped as it does not work with ').concat(scaleType," scale.")}function stepDropped(channel){return'The step for "'.concat(channel,'" is dropped because the ').concat("width"===channel?"x":"y"," is continuous.")}function cannotStackRangedMark(channel){return'Cannot stack "'.concat(channel,'" if there is already "').concat(channel,'2".')}function invalidTimeUnit(unitName,value){return"Invalid ".concat(unitName,": ").concat(stringify(value),".")}function errorBand1DNotSupport(property){return"1D error band does not support ".concat(property,
".")}function channelRequiredForBinned(channel){return"Channel ".concat(channel,' is required for "binned" bin.')}function warn(...args){current.warn(...args)}function isDateTime(o){if(o&&isObject(o))for(const part of TIMEUNIT_PARTS)if(part in o)return!0;return!1}function normalizeMonth(m){isNumeric(m)&&(m=+m);if(isNumber(m))return m-1;{var lowerM=m.toLowerCase();const monthIndex=MONTHS.indexOf(lowerM);if(-1!==monthIndex)return monthIndex;lowerM=lowerM.substr(0,3);lowerM=SHORT_MONTHS.indexOf(lowerM);
if(-1!==lowerM)return lowerM;throw Error(invalidTimeUnit("month",m));}}function normalizeDay(d){isNumeric(d)&&(d=+d);if(isNumber(d))return d%7;{var lowerD=d.toLowerCase();const dayIndex=DAYS.indexOf(lowerD);if(-1!==dayIndex)return dayIndex;lowerD=lowerD.substr(0,3);lowerD=SHORT_DAYS.indexOf(lowerD);if(-1!==lowerD)return lowerD;throw Error(invalidTimeUnit("day",d));}}function dateTimeParts(d,normalize){const parts=[];normalize&&void 0!==d.day&&1<keys$jscomp$0(d).length&&(warn("Dropping day from datetime ".concat(stringify(d),
" as day cannot be combined with other units.")),d=duplicate(d),delete d.day);void 0!==d.year?parts.push(d.year):parts.push(2012);if(void 0!==d.month){var month=normalize?normalizeMonth(d.month):d.month;parts.push(month)}else if(void 0!==d.quarter){if(normalize)if(month=d.quarter,isNumeric(month)&&(month=+month),isNumber(month))4<month&&warn(invalidTimeUnit("quarter",month)),--month;else throw Error(invalidTimeUnit("quarter",month));else month=d.quarter;parts.push(isNumber(month)?3*month:month+"*3")}else parts.push(0);
void 0!==d.date?parts.push(d.date):void 0!==d.day?(normalize=normalize?normalizeDay(d.day):d.day,parts.push(isNumber(normalize)?normalize+1:normalize+"+1")):parts.push(1);for(const timeUnit of["hours","minutes","seconds","milliseconds"])normalize=d[timeUnit],parts.push("undefined"===typeof normalize?0:normalize);return parts}function dateTimeToExpr(d){const string=dateTimeParts(d,!0).join(", ");return d.utc?"utc(".concat(string,")"):"datetime(".concat(string,")")}function dateTimeToTimestamp(d){const parts=
dateTimeParts(d,!0);return d.utc?+new Date(Date.UTC(...parts)):+new Date(...parts)}function getTimeUnitParts(timeUnit){const parts=[];for(const part of TIMEUNIT_PARTS)containsTimeUnit(timeUnit,part)&&parts.push(part);return parts}function containsTimeUnit(fullTimeUnit,timeUnit){const index=fullTimeUnit.indexOf(timeUnit);return 0>index||0<index&&"seconds"===timeUnit&&"i"===fullTimeUnit.charAt(index-1)||fullTimeUnit.length>index+3&&"day"===timeUnit&&"o"===fullTimeUnit.charAt(index+3)||0<index&&"year"===
timeUnit&&"f"===fullTimeUnit.charAt(index-1)?!1:!0}function timeUnitSpecifierExpression(timeUnit){if(timeUnit)return timeUnit=getTimeUnitParts(timeUnit),"timeUnitSpecifier(".concat(fastJsonStableStringify(timeUnit),", ").concat(fastJsonStableStringify(VEGALITE_TIMEFORMAT),")")}function normalizeTimeUnit(timeUnit){if(timeUnit){var params;isString(timeUnit)?params={unit:timeUnit}:isObject(timeUnit)&&(params=Object.assign({},timeUnit,timeUnit.unit?{unit:timeUnit.unit}:{}));params.unit.startsWith("utc")&&
(params.utc=!0,params.unit=params.unit.substr(3));return params}}function timeUnitToString(tu){var $jscomp$destructuring$var13=normalizeTimeUnit(tu);tu=Object.assign({},$jscomp$destructuring$var13);$jscomp$destructuring$var13=$jscomp$destructuring$var13.utc;const rest=(delete tu.utc,tu);return rest.unit?($jscomp$destructuring$var13?"utc":"")+keys$jscomp$0(rest).map(p=>varName("".concat("unit"===p?"":"_".concat(p,"_")).concat(rest[p]))).join(""):($jscomp$destructuring$var13?"utc":"")+"timeunit"+keys$jscomp$0(rest).map(p=>
varName("_".concat(p,"_").concat(rest[p]))).join("")}function isSignalRef(o){return o&&!!o.signal}function isVgRangeStep(range){return!!range.step}function isDataRefDomain(domain){return isArray(domain)?!1:"field"in domain&&"data"in domain}function isFieldEqualPredicate(predicate){return predicate&&!!predicate.field&&void 0!==predicate.equal}function isFieldLTPredicate(predicate){return predicate&&!!predicate.field&&void 0!==predicate.lt}function isFieldLTEPredicate(predicate){return predicate&&!!predicate.field&&
void 0!==predicate.lte}function isFieldGTPredicate(predicate){return predicate&&!!predicate.field&&void 0!==predicate.gt}function isFieldGTEPredicate(predicate){return predicate&&!!predicate.field&&void 0!==predicate.gte}function isFieldRangePredicate(predicate){return predicate&&predicate.field&&(isArray(predicate.range)&&2===predicate.range.length||isSignalRef(predicate.range))?!0:!1}function isFieldOneOfPredicate(predicate){return predicate&&!!predicate.field&&(isArray(predicate.oneOf)||isArray(predicate.in))}
function isFieldPredicate(predicate){return isFieldOneOfPredicate(predicate)||isFieldEqualPredicate(predicate)||isFieldRangePredicate(predicate)||isFieldLTPredicate(predicate)||isFieldGTPredicate(predicate)||isFieldLTEPredicate(predicate)||isFieldGTEPredicate(predicate)}function predicateValueExpr(v,timeUnit){return valueExpr(v,{timeUnit,wrapTime:!0})}function predicateValuesExpr(vals,timeUnit){return vals.map(v=>predicateValueExpr(v,timeUnit))}function fieldFilterExpression(predicate,useInRange){useInRange=
void 0===useInRange?!0:useInRange;var _normalizeTimeUnit,{field}=predicate;const timeUnit=null===(_normalizeTimeUnit=normalizeTimeUnit(predicate.timeUnit))||void 0===_normalizeTimeUnit?void 0:_normalizeTimeUnit.unit;if(timeUnit){{({end:_normalizeTimeUnit}={end:!1});field=accessPathWithDatum(field);const utc=timeUnit.startsWith("utc")?"utc":"";const dateExpr={};for(JSCompiler_temp of TIMEUNIT_PARTS)if(containsTimeUnit(timeUnit,JSCompiler_temp)){var lastTimeUnit=JSCompiler_temp;lastTimeUnit="quarter"===
lastTimeUnit?"(".concat(utc,"quarter(").concat(field,")-1)"):"".concat(utc).concat(lastTimeUnit,"(").concat(field,")");dateExpr[JSCompiler_temp]=lastTimeUnit;lastTimeUnit=JSCompiler_temp}_normalizeTimeUnit&&(dateExpr[lastTimeUnit]+="+1");var JSCompiler_temp=dateExpr;_normalizeTimeUnit=dateTimeParts(JSCompiler_temp,!1).join(", ");JSCompiler_temp=JSCompiler_temp.utc?"utc(".concat(_normalizeTimeUnit,")"):"datetime(".concat(_normalizeTimeUnit,")")}JSCompiler_temp="time("+JSCompiler_temp+")"}else JSCompiler_temp=
vgField(predicate,{expr:"datum"});if(isFieldEqualPredicate(predicate))return JSCompiler_temp+"\x3d\x3d\x3d"+predicateValueExpr(predicate.equal,timeUnit);if(isFieldLTPredicate(predicate))return useInRange=predicate.lt,"".concat(JSCompiler_temp,"\x3c").concat(predicateValueExpr(useInRange,timeUnit));if(isFieldGTPredicate(predicate))return useInRange=predicate.gt,"".concat(JSCompiler_temp,"\x3e").concat(predicateValueExpr(useInRange,timeUnit));if(isFieldLTEPredicate(predicate))return useInRange=predicate.lte,
"".concat(JSCompiler_temp,"\x3c\x3d").concat(predicateValueExpr(useInRange,timeUnit));if(isFieldGTEPredicate(predicate))return useInRange=predicate.gte,"".concat(JSCompiler_temp,"\x3e\x3d").concat(predicateValueExpr(useInRange,timeUnit));if(isFieldOneOfPredicate(predicate))return"indexof([".concat(predicateValuesExpr(predicate.oneOf,timeUnit).join(","),"], ").concat(JSCompiler_temp,") !\x3d\x3d -1");if(predicate&&predicate.field&&void 0!==predicate.valid)return fieldValidPredicate(JSCompiler_temp,
predicate.valid);if(isFieldRangePredicate(predicate)){({range:_normalizeTimeUnit}=predicate);predicate=isSignalRef(_normalizeTimeUnit)?{signal:"".concat(_normalizeTimeUnit.signal,"[0]")}:_normalizeTimeUnit[0];_normalizeTimeUnit=isSignalRef(_normalizeTimeUnit)?{signal:"".concat(_normalizeTimeUnit.signal,"[1]")}:_normalizeTimeUnit[1];if(null!==predicate&&null!==_normalizeTimeUnit&&useInRange)return"inrange("+JSCompiler_temp+", ["+predicateValueExpr(predicate,timeUnit)+", "+predicateValueExpr(_normalizeTimeUnit,
timeUnit)+"])";useInRange=[];null!==predicate&&useInRange.push("".concat(JSCompiler_temp," \x3e\x3d ").concat(predicateValueExpr(predicate,timeUnit)));null!==_normalizeTimeUnit&&useInRange.push("".concat(JSCompiler_temp," \x3c\x3d ").concat(predicateValueExpr(_normalizeTimeUnit,timeUnit)));return 0<useInRange.length?useInRange.join(" \x26\x26 "):"true"}throw Error("Invalid field predicate: ".concat(JSON.stringify(predicate)));}function fieldValidPredicate(fieldExpr,valid){return void 0===valid||valid?
"isValid(".concat(fieldExpr,") \x26\x26 isFinite(+").concat(fieldExpr,")"):"!isValid(".concat(fieldExpr,") || !isFinite(+").concat(fieldExpr,")")}function normalizePredicate(f){if(isFieldPredicate(f)&&f.timeUnit){var _normalizeTimeUnit2;return Object.assign({},f,{timeUnit:null===(_normalizeTimeUnit2=normalizeTimeUnit(f.timeUnit))||void 0===_normalizeTimeUnit2?void 0:_normalizeTimeUnit2.unit})}return f}function getFullName(type){if(type)switch(type=type.toLowerCase(),type){case "q":case "quantitative":return"quantitative";
...
"band","identity","ordinal"],scaleType);case "round":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX||"band"===scaleType||"point"===scaleType;case "padding":case "rangeMin":case "rangeMax":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX||contains(["point","band"],scaleType);case "paddingOuter":case "align":return contains(["point","band"],scaleType);case "paddingInner":return"band"===scaleType;case "domainMax":case "domainMid":case "domainMin":case "clamp":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX;
case "nice":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX||"quantize"===scaleType||"threshold"===scaleType;case "exponent":return"pow"===scaleType;case "base":return"log"===scaleType;case "constant":return"symlog"===scaleType;case "zero":return scaleType in CONTINUOUS_DOMAIN_INDEX&&!contains(["log","time","utc","threshold","quantile"],scaleType)}}function channelScalePropertyIncompatability(channel,propName){switch(propName){case "interpolate":case "scheme":case "domainMid":if(!isColorChannel(channel))return'Cannot use the scale property "'.concat(channel,
'" with non-color channel.')}}function channelSupportScaleType(channel,scaleType){if(!SCALE_CHANNEL_INDEX[channel])return!1;switch(channel){case "x":case "y":case "theta":case "radius":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX||contains(["band","point"],scaleType);case "size":case "strokeWidth":case "opacity":case "fillOpacity":case "strokeOpacity":case "angle":return scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX||scaleType in CONTINUOUS_TO_DISCRETE_INDEX||contains(["band","point","ordinal"],
scaleType);case "color":case "fill":case "stroke":return"band"!==scaleType;case "strokeDash":return"ordinal"===scaleType||scaleType in CONTINUOUS_TO_DISCRETE_INDEX;case "shape":return"ordinal"===scaleType}}function isExprRef(o){return o&&!!o.expr}function replaceExprRefInIndex(index){const props=keys$jscomp$0(index||{}),newIndex={};for(const prop of props)newIndex[prop]=signalRefOrValue(index[prop]);return newIndex}function isPathMark(m){return contains(["line","area","trail"],m)}function isMarkDef(mark){return mark.type}
function midPointRefWithPositionInvalidTest(params){const {channel,channelDef,markDef,scale,config}=params;params=midPoint(params);return isFieldDef(channelDef)&&!isCountingAggregateOp(channelDef.aggregate)&&scale&&scale.get("type")in CONTINUOUS_TO_CONTINUOUS_INDEX&&!1===scale.get("zero")?wrapPositionInvalidTest({fieldDef:channelDef,channel,markDef,ref:params,config}):params}function wrapPositionInvalidTest($jscomp$destructuring$var17){var {fieldDef,channel,markDef,ref,config}=$jscomp$destructuring$var17;
if(isPathMark(markDef.type)||null===getMarkPropOrConfig("invalid",markDef,config))return ref;{$jscomp$destructuring$var17=fieldInvalidPredicate(fieldDef,!0);const zeroValueRef="y"===getMainRangeChannel(channel)?{field:{group:"height"}}:{value:0};$jscomp$destructuring$var17=Object.assign({},{test:$jscomp$destructuring$var17},zeroValueRef)}return[$jscomp$destructuring$var17,ref]}function fieldInvalidPredicate(field,invalid){invalid=void 0===invalid?!0:invalid;return fieldValidPredicate(isString(field)?
field:vgField(field,{expr:"datum"}),!invalid)}function valueRefForFieldOrDatumDef(fieldDef,scaleName,opt,encode){const ref={};scaleName&&(ref.scale=scaleName);isDatumDef(fieldDef)?({datum:fieldDef}=fieldDef,isDateTime(fieldDef)?ref.signal=dateTimeToExpr(fieldDef):isSignalRef(fieldDef)?ref.signal=fieldDef.signal:isExprRef(fieldDef)?ref.signal=fieldDef.expr:ref.value=fieldDef):ref.field=vgField(fieldDef,opt);if(encode){const {offset,band}=encode;offset&&(ref.offset=offset);band&&(ref.band=band)}return ref}
function interpolatedSignalRef($jscomp$destructuring$var18){var {scaleName,fieldOrDatumDef,fieldOrDatumDef2,offset,startSuffix,band=.5}=$jscomp$destructuring$var18,expr=0<band&&1>band?"datum":void 0;$jscomp$destructuring$var18=vgField(fieldOrDatumDef,{expr,suffix:startSuffix});const end=void 0!==fieldOrDatumDef2?vgField(fieldOrDatumDef2,{expr}):vgField(fieldOrDatumDef,{suffix:"end",expr});expr={};0===band||1===band?(expr.scale=scaleName,expr.field=0===band?$jscomp$destructuring$var18:end):($jscomp$destructuring$var18=
"".concat(band," * ").concat($jscomp$destructuring$var18," + ").concat(1-band," * ").concat(end),expr.signal='scale("'.concat(scaleName,'", ').concat($jscomp$destructuring$var18,")"));offset&&(expr.offset=offset);return expr}function midPoint($jscomp$destructuring$var19){var {channel,channelDef,channel2Def,markDef,config,scaleName,scale,stack,offset,defaultRef,band}=$jscomp$destructuring$var19;if(channelDef){if(isFieldOrDatumDef(channelDef)){var _ref,_band2;if(isTypedFieldDef(channelDef)){var _band;
band=null!==(_band=band)&&void 0!==_band?_band:getBand({channel,fieldDef:channelDef,fieldDef2:channel2Def,markDef,stack,config,isMidPoint:!0});const {bin,timeUnit,type}=channelDef;if(isBinning(bin)||band&&timeUnit&&"temporal"===type)return stack&&stack.impute?valueRefForFieldOrDatumDef(channelDef,scaleName,{binSuffix:"mid"},{offset}):band?interpolatedSignalRef({scaleName,fieldOrDatumDef:channelDef,band,offset}):valueRefForFieldOrDatumDef(channelDef,scaleName,binRequiresRange(channelDef,channel)?{binSuffix:"range"}:
{},{offset});if(isBinned(bin)){if(isFieldDef(channel2Def))return interpolatedSignalRef({scaleName,fieldOrDatumDef:channelDef,fieldOrDatumDef2:channel2Def,band,offset});warn(channelRequiredForBinned("x"===channel?"x2":"y2"))}}$jscomp$destructuring$var19=null===scale||void 0===scale?void 0:scale.get("type");return valueRefForFieldOrDatumDef(channelDef,scaleName,$jscomp$destructuring$var19 in DISCRETE_DOMAIN_INDEX?{binSuffix:"range"}:{},{offset,band:"band"===$jscomp$destructuring$var19?null!==(_ref=
null!==(_band2=band)&&void 0!==_band2?_band2:channelDef.band)&&void 0!==_ref?_ref:.5:void 0})}if(isValueDef(channelDef))return _ref=offset?{offset}:{},Object.assign({},widthHeightValueOrSignalRef(channel,channelDef.value),_ref)}"function"===typeof defaultRef&&(defaultRef=defaultRef());return defaultRef?Object.assign({},defaultRef,offset?{offset}:{}):defaultRef}function widthHeightValueOrSignalRef(channel,value){return contains(["x","x2"],channel)&&"width"===value?{field:{group:"width"}}:contains(["y",
"y2"],channel)&&"height"===value?{field:{group:"height"}}:signalOrValueRef(value)}function isCustomFormatType(formatType){return formatType&&"number"!==formatType&&"time"!==formatType}function customFormatExpr(formatType,field,format){return"".concat(formatType,"(").concat(field).concat(format?", ".concat(JSON.stringify(format)):"",")")}function formatSignalRef($jscomp$destructuring$var20){var {fieldOrDatumDef,format,formatType,expr,normalizeStack,config}=$jscomp$destructuring$var20;if(isCustomFormatType(formatType))return formatCustomType({fieldOrDatumDef,
format,formatType,expr,config});$jscomp$destructuring$var20=fieldToFormat(fieldOrDatumDef,expr,normalizeStack);if(isFieldOrDatumDefForTimeFormat(fieldOrDatumDef)){var _normalizeTimeUnit,_fieldOrDatumDef$scal;var timeUnit=isFieldDef(fieldOrDatumDef)?null===(_normalizeTimeUnit=normalizeTimeUnit(fieldOrDatumDef.timeUnit))||void 0===_normalizeTimeUnit?void 0:_normalizeTimeUnit.unit:void 0,format$jscomp$0=format,rawTimeFormat=config.timeFormat;_normalizeTimeUnit=isScaleFieldDef(fieldOrDatumDef)&&(null===
(_fieldOrDatumDef$scal=fieldOrDatumDef.scale)||void 0===_fieldOrDatumDef$scal?void 0:_fieldOrDatumDef$scal.type)===ScaleType.UTC;!timeUnit||format$jscomp$0?(format$jscomp$0=isString(format$jscomp$0)?format$jscomp$0:rawTimeFormat,$jscomp$destructuring$var20="".concat(_normalizeTimeUnit?"utc":"time","Format(").concat($jscomp$destructuring$var20,", '").concat(format$jscomp$0,"')")):timeUnit?(_fieldOrDatumDef$scal=timeUnitSpecifierExpression(timeUnit),timeUnit=_normalizeTimeUnit||timeUnit.startsWith("utc"),
$jscomp$destructuring$var20="".concat(timeUnit?"utc":"time","Format(").concat($jscomp$destructuring$var20,", ").concat(_fieldOrDatumDef$scal,")")):$jscomp$destructuring$var20=void 0;return $jscomp$destructuring$var20?{signal:$jscomp$destructuring$var20}:void 0}format=numberFormat(channelDefType(fieldOrDatumDef),format,config);return isFieldDef(fieldOrDatumDef)&&isBinning(fieldOrDatumDef.bin)?(_fieldOrDatumDef$scal=vgField(fieldOrDatumDef,{expr,binSuffix:"end"}),{signal:binFormatExpression($jscomp$destructuring$var20,
_fieldOrDatumDef$scal,format,formatType,config)}):format||"quantitative"===channelDefType(fieldOrDatumDef)?{signal:"".concat(formatExpr($jscomp$destructuring$var20,format))}:{signal:"isValid(".concat($jscomp$destructuring$var20,") ? ").concat($jscomp$destructuring$var20,' : ""+').concat($jscomp$destructuring$var20)}}function fieldToFormat(fieldOrDatumDef,expr,normalizeStack){if(isFieldDef(fieldOrDatumDef))return normalizeStack?"".concat(vgField(fieldOrDatumDef,{expr,suffix:"end"}),"-").concat(vgField(fieldOrDatumDef,
{expr,suffix:"start"})):vgField(fieldOrDatumDef,{expr});({datum:fieldOrDatumDef}=fieldOrDatumDef);fieldOrDatumDef=isDateTime(fieldOrDatumDef)?dateTimeToExpr(fieldOrDatumDef):"".concat(JSON.stringify(fieldOrDatumDef));return fieldOrDatumDef}function formatCustomType($jscomp$destructuring$var21){var {fieldOrDatumDef,format,formatType,expr,normalizeStack,config,field}=$jscomp$destructuring$var21,_field;field=null!==(_field=field)&&void 0!==_field?_field:fieldToFormat(fieldOrDatumDef,expr,normalizeStack);
return isFieldDef(fieldOrDatumDef)&&isBinning(fieldOrDatumDef.bin)?($jscomp$destructuring$var21=vgField(fieldOrDatumDef,{expr,binSuffix:"end"}),{signal:binFormatExpression(field,$jscomp$destructuring$var21,format,formatType,config)}):{signal:customFormatExpr(formatType,field,format)}}function guideFormat(fieldOrDatumDef,type,format,formatType,config,omitTimeFormatConfig){if(!isCustomFormatType(formatType)){if(isFieldOrDatumDefForTimeFormat(fieldOrDatumDef)){var _normalizeTimeUnit2;fieldOrDatumDef=
isFieldDef(fieldOrDatumDef)?null===(_normalizeTimeUnit2=normalizeTimeUnit(fieldOrDatumDef.timeUnit))||void 0===_normalizeTimeUnit2?void 0:_normalizeTimeUnit2.unit:void 0;format=format?format:fieldOrDatumDef?{signal:timeUnitSpecifierExpression(fieldOrDatumDef)}:omitTimeFormatConfig?void 0:config.timeFormat;return format}return numberFormat(type,format,config)}}function guideFormatType(formatType,fieldOrDatumDef,scaleType){if(formatType&&(isSignalRef(formatType)||"number"===formatType||"time"===formatType))return formatType;
if(isFieldOrDatumDefForTimeFormat(fieldOrDatumDef)&&"time"!==scaleType&&"utc"!==scaleType)return"time"}function numberFormat(type,specifiedFormat,config){if(isString(specifiedFormat))return specifiedFormat;if("quantitative"===type)return config.numberFormat}function formatExpr(field,format){return"format(".concat(field,', "').concat(format||"",'")')}function binNumberFormatExpr(field,format,formatType,config){var _ref;return isCustomFormatType(formatType)?customFormatExpr(formatType,field,format):
formatExpr(field,null!==(_ref=isString(format)?format:void 0)&&void 0!==_ref?_ref:config.numberFormat)}function binFormatExpression(startField,endField,format,formatType,config){const start=binNumberFormatExpr(startField,format,formatType,config);endField=binNumberFormatExpr(endField,format,formatType,config);return"".concat(fieldValidPredicate(startField,!1),' ? "null" : ').concat(start,' + "').concat(" – ",'" + ').concat(endField)}function isSortField(sort){return!!sort&&("count"===sort.op||!!sort.field)}
function isSortArray(sort){return!!sort&&isArray(sort)}function isFacetMapping(f){return"row"in f||"column"in f}function toFieldDefBase(fieldDef){const {field,timeUnit,bin,aggregate}=fieldDef;return Object.assign({},timeUnit?{timeUnit}:{},bin?{bin}:{},aggregate?{aggregate}:{},{field})}function getBand($jscomp$destructuring$var22){var {channel,fieldDef,fieldDef2,markDef:mark,stack,config,isMidPoint}=$jscomp$destructuring$var22;if(isFieldOrDatumDef(fieldDef)&&void 0!==fieldDef.band)return fieldDef.band;
if(isFieldDef(fieldDef)){const {timeUnit,bin}=fieldDef;if(timeUnit&&!fieldDef2)return isMidPoint?getMarkConfig("timeUnitBandPosition",mark,config):contains(["rect","bar","image","arc"],mark.type)?getMarkConfig("timeUnitBand",mark,config):0;if(isBinning(bin))return contains(["rect","bar","image","arc"],mark.type)&&!isMidPoint?1:.5}if((null===stack||void 0===stack?void 0:stack.fieldChannel)===channel&&isMidPoint)return.5}function hasBand(channel,fieldDef,fieldDef2,stack,markDef,config){return isBinning(fieldDef.bin)||
fieldDef.timeUnit&&isTypedFieldDef(fieldDef)&&"temporal"===fieldDef.type?!!getBand({channel,fieldDef,fieldDef2,stack,markDef,config}):!1}function hasConditionalFieldDef(channelDef){channelDef=channelDef&&channelDef.condition;return!!channelDef&&!isArray(channelDef)&&isFieldDef(channelDef)}function hasConditionalFieldOrDatumDef(channelDef){channelDef=channelDef&&channelDef.condition;return!!channelDef&&!isArray(channelDef)&&isFieldOrDatumDef(channelDef)}function isFieldDef(channelDef){return!!channelDef&&
(!!channelDef.field||"count"===channelDef.aggregate)}function channelDefType(channelDef){return channelDef&&channelDef.type}function isDatumDef(channelDef){return!!channelDef&&"datum"in channelDef}function isContinuousFieldOrDatumDef(cd){return isTypedFieldDef(cd)&&!isDiscrete(cd)||isNumericDataDef(cd)}function isNumericDataDef(cd){return isDatumDef(cd)&&isNumber(cd.datum)}function isFieldOrDatumDef(channelDef){return isFieldDef(channelDef)||isDatumDef(channelDef)}function isTypedFieldDef(channelDef){return!!channelDef&&
("field"in channelDef||"count"===channelDef.aggregate)&&"type"in channelDef}function isValueDef(channelDef){return channelDef&&"value"in channelDef&&"value"in channelDef}function isScaleFieldDef(channelDef){return!!channelDef&&("scale"in channelDef||"sort"in channelDef)}function isPositionFieldOrDatumDef(channelDef){return channelDef&&("axis"in channelDef||"stack"in channelDef||"impute"in channelDef)}function isStringFieldOrDatumDef(channelDef){return!!channelDef&&("format"in channelDef||"formatType"in
channelDef)}function toStringFieldDef(fieldDef){return omit(fieldDef,["legend","axis","header","scale"])}function vgField(fieldDef,opt){opt=void 0===opt?{}:opt;let field=fieldDef.field;const prefix=opt.prefix;let suffix=opt.suffix,argAccessor="";if("count"===fieldDef.aggregate)field=internalField("count");else{let fn;if(!opt.nofn)if("op"in fieldDef)fn=fieldDef.op;else{const {bin,aggregate,timeUnit}=fieldDef;if(isBinning(bin)){var _opt$binSuffix,_opt$suffix;fn=binToString(bin);suffix=(null!==(_opt$binSuffix=
opt.binSuffix)&&void 0!==_opt$binSuffix?_opt$binSuffix:"")+(null!==(_opt$suffix=opt.suffix)&&void 0!==_opt$suffix?_opt$suffix:"")}else if(aggregate)isArgmaxDef(aggregate)?(argAccessor='["'.concat(field,'"]'),field="argmax_".concat(aggregate.argmax)):isArgminDef(aggregate)?(argAccessor='["'.concat(field,'"]'),field="argmin_".concat(aggregate.argmin)):fn=String(aggregate);else if(timeUnit){var _opt$suffix2;fn=timeUnitToString(timeUnit);suffix=(!contains(["range","mid"],opt.binSuffix)&&opt.binSuffix||
"")+(null!==(_opt$suffix2=opt.suffix)&&void 0!==_opt$suffix2?_opt$suffix2:"")}}fn&&(field=field?"".concat(fn,"_").concat(field):fn)}suffix&&(field="".concat(field,"_").concat(suffix));prefix&&(field="".concat(prefix,"_").concat(field));return opt.forAs?removePathFromField(field):opt.expr?flatAccessWithDatum(field,opt.expr)+argAccessor:replacePathInField(field)+argAccessor}function isDiscrete(def){switch(def.type){case "nominal":case "ordinal":case "geojson":return!0;case "quantitative":return isFieldDef(def)&&
!!def.bin;case "temporal":return!1}throw Error('Invalid field type "'.concat(def.type,'".'));}function verbalTitleFormatter(fieldDef,config){const {field,bin,timeUnit,aggregate}=fieldDef;if("count"===aggregate)return config.countTitle;if(isBinning(bin))return"".concat(field," (binned)");if(timeUnit){var _normalizeTimeUnit;if(fieldDef=null===(_normalizeTimeUnit=normalizeTimeUnit(timeUnit))||void 0===_normalizeTimeUnit?void 0:_normalizeTimeUnit.unit)return"".concat(field," (").concat(getTimeUnitParts(fieldDef).join("-"),
")")}else if(aggregate)return isArgmaxDef(aggregate)?"".concat(field," for max ").concat(aggregate.argmax):isArgminDef(aggregate)?"".concat(field," for min ").concat(aggregate.argmin):"".concat(titleCase(aggregate)," of ").concat(field);return field}function title$jscomp$0(fieldOrDatumDef,config,$jscomp$destructuring$var23){var {allowDisabling,includeDefault=!0}=$jscomp$destructuring$var23,_getGuide;$jscomp$destructuring$var23=null===(_getGuide=getGuide(fieldOrDatumDef))||void 0===_getGuide?void 0:
_getGuide.title;if(!isFieldDef(fieldOrDatumDef))return $jscomp$destructuring$var23;config=includeDefault?titleFormatter(fieldOrDatumDef,config):void 0;if(allowDisabling)return getFirstDefined($jscomp$destructuring$var23,fieldOrDatumDef.title,config);var _ref;return null!==(_ref=null!==$jscomp$destructuring$var23&&void 0!==$jscomp$destructuring$var23?$jscomp$destructuring$var23:fieldOrDatumDef.title)&&void 0!==_ref?_ref:config}function getGuide(fieldDef){if(isPositionFieldOrDatumDef(fieldDef)&&fieldDef.axis)return fieldDef.axis;
if(fieldDef&&"legend"in fieldDef&&fieldDef.legend)return fieldDef.legend;if(fieldDef&&"header"in fieldDef&&fieldDef.header)return fieldDef.header}function getFormatMixins(fieldDef){if(isStringFieldOrDatumDef(fieldDef)){const {format,formatType}=fieldDef;return{format,formatType}}{var _getGuide2;fieldDef=null!==(_getGuide2=getGuide(fieldDef))&&void 0!==_getGuide2?_getGuide2:{};const {format,formatType}=fieldDef;return{format,formatType}}}function defaultType(fieldDef,channel){var _fieldDef$scale;switch(channel){case "latitude":case "longitude":return"quantitative";
case "row":case "column":case "facet":case "shape":case "strokeDash":return"nominal";case "order":return"ordinal"}if("sort"in fieldDef&&isArray(fieldDef.sort))return"ordinal";const {aggregate,bin,timeUnit}=fieldDef;if(timeUnit)return"temporal";if(bin||aggregate&&!isArgmaxDef(aggregate)&&!isArgminDef(aggregate))return"quantitative";if(isScaleFieldDef(fieldDef)&&(null===(_fieldDef$scale=fieldDef.scale)||void 0===_fieldDef$scale?0:_fieldDef$scale.type))switch(SCALE_CATEGORY_INDEX[fieldDef.scale.type]){case "numeric":case "discretizing":return"quantitative";
case "time":return"temporal"}return"nominal"}function getFieldDef(channelDef){if(isFieldDef(channelDef))return channelDef;if(hasConditionalFieldDef(channelDef))return channelDef.condition}function getFieldOrDatumDef(channelDef){if(isFieldOrDatumDef(channelDef))return channelDef;if(hasConditionalFieldOrDatumDef(channelDef))return channelDef.condition}function initChannelDef(channelDef,channel,config,opt){opt=void 0===opt?{}:opt;return isString(channelDef)||isNumber(channelDef)||isBoolean(channelDef)?
(config=isString(channelDef)?"string":isNumber(channelDef)?"number":"boolean",channel="Channel ".concat(channel," is a ").concat(config,". Converted to {value: ").concat(stringify(channelDef),"}."),warn(channel),{value:channelDef}):isFieldOrDatumDef(channelDef)?initFieldOrDatumDef(channelDef,channel,config,opt):hasConditionalFieldOrDatumDef(channelDef)?Object.assign({},channelDef,{condition:initFieldOrDatumDef(channelDef.condition,channel,config,opt)}):channelDef}function initFieldOrDatumDef(fd,channel,
config,opt){if(isStringFieldOrDatumDef(fd)){var $jscomp$destructuring$var25=Object.assign({},fd),formatType=fd.formatType;$jscomp$destructuring$var25=(delete $jscomp$destructuring$var25.format,delete $jscomp$destructuring$var25.formatType,$jscomp$destructuring$var25);if(isCustomFormatType(formatType)&&!config.customFormatTypes)return warn(customFormatTypeNotAllowed(channel)),initFieldOrDatumDef($jscomp$destructuring$var25,channel,config,opt)}else if((formatType=isPositionFieldOrDatumDef(fd)?"axis":
fd&&"legend"in fd?"legend":fd&&"header"in fd?"header":null)&&fd[formatType]){var $jscomp$destructuring$var26=fd[formatType];$jscomp$destructuring$var25=Object.assign({},$jscomp$destructuring$var26);$jscomp$destructuring$var26=$jscomp$destructuring$var26.formatType;$jscomp$destructuring$var25=(delete $jscomp$destructuring$var25.format,delete $jscomp$destructuring$var25.formatType,$jscomp$destructuring$var25);if(isCustomFormatType($jscomp$destructuring$var26)&&!config.customFormatTypes)return warn(customFormatTypeNotAllowed(channel)),
initFieldOrDatumDef(Object.assign({},fd,{[formatType]:$jscomp$destructuring$var25}),channel,config,opt)}if(isFieldDef(fd))return initFieldDef(fd,channel,opt);channel=fd.type;channel||({datum:channel}=fd,channel=isNumber(channel)?"quantitative":isString(channel)?"nominal":isDateTime(channel)?"temporal":void 0,fd=Object.assign({},fd,{type:channel}));return fd}function initFieldDef(fd,channel,$jscomp$destructuring$var28){({compositeMark:$jscomp$destructuring$var28=!1}=void 0===$jscomp$destructuring$var28?
{}:$jscomp$destructuring$var28);const {aggregate,timeUnit,bin,field}=fd;fd=Object.assign({},fd);$jscomp$destructuring$var28||!aggregate||isAggregateOp(aggregate)||isArgmaxDef(aggregate)||isArgminDef(aggregate)||(warn('Invalid aggregation operator "'.concat(aggregate,'".')),delete fd.aggregate);timeUnit&&(fd.timeUnit=normalizeTimeUnit(timeUnit));field&&(fd.field="".concat(field));isBinning(bin)&&(fd.bin=normalizeBin(bin,channel));!isBinned(bin)||channel in POSITION_SCALE_CHANNEL_INDEX||warn("Channel ".concat(channel,
' should not be used with "binned" bin.'));if(isTypedFieldDef(fd)){({type:$jscomp$destructuring$var28}=fd);var fullType=getFullName($jscomp$destructuring$var28);$jscomp$destructuring$var28!==fullType&&(fd.type=fullType);"quantitative"!==$jscomp$destructuring$var28&&isCountingAggregateOp(aggregate)&&($jscomp$destructuring$var28='Invalid field type "'.concat($jscomp$destructuring$var28,'" for aggregate: "').concat(aggregate,'", using "quantitative" instead.'),warn($jscomp$destructuring$var28),fd.type=
"quantitative")}else isSecondaryRangeChannel(channel)||($jscomp$destructuring$var28=defaultType(fd,channel),fd.type=$jscomp$destructuring$var28);if(isTypedFieldDef(fd)){const {compatible,warning}=channelCompatibility(fd,channel)||{};!1===compatible&&warn(warning)}if("sort"in fd&&isString(fd.sort)){({sort:channel}=fd);if(channel in SORT_BY_CHANNEL_INDEX)return Object.assign({},fd,{sort:{encoding:channel}});$jscomp$destructuring$var28=channel.substr(1);if("-"===channel.charAt(0)&&$jscomp$destructuring$var28 in
SORT_BY_CHANNEL_INDEX)return Object.assign({},fd,{sort:{encoding:$jscomp$destructuring$var28,order:"descending"}})}return fd&&"header"in fd&&({header:channel}=fd,fullType=channel,$jscomp$destructuring$var28=Object.assign({},fullType),fullType=fullType.orient,$jscomp$destructuring$var28=(delete $jscomp$destructuring$var28.orient,$jscomp$destructuring$var28),fullType)?Object.assign({},fd,{header:Object.assign({},$jscomp$destructuring$var28,{labelOrient:channel.labelOrient||fullType,titleOrient:channel.titleOrient||
fullType})}):fd}function normalizeBin(bin,channel){return isBoolean(bin)?{maxbins:autoMaxBins(channel)}:"binned"===bin?{binned:!0}:bin.maxbins||bin.step?bin:Object.assign({},bin,{maxbins:autoMaxBins(channel)})}function channelCompatibility(fieldDef,channel){const type=fieldDef.type;if("geojson"===type&&"shape"!==channel)return{compatible:!1,warning:"Channel ".concat(channel," should not be used with a geojson data.")};switch(channel){case "row":case "column":case "facet":return isDiscrete(fieldDef)?
COMPATIBLE:{compatible:!1,warning:"".concat(channel," encoding should be discrete (ordinal / nominal / binned).")};case "x":case "y":case "color":case "fill":case "stroke":case "text":case "detail":case "key":case "tooltip":case "href":case "url":case "angle":case "theta":case "radius":case "description":return COMPATIBLE;case "longitude":case "longitude2":case "latitude":case "latitude2":return"quantitative"!==type?{compatible:!1,warning:"Channel ".concat(channel," should be used with a quantitative field only, not ").concat(fieldDef.type,
" field.")}:COMPATIBLE;case "opacity":case "fillOpacity":case "strokeOpacity":case "strokeWidth":case "size":case "theta2":case "radius2":case "x2":case "y2":return"nominal"!==type||fieldDef.sort?COMPATIBLE:{compatible:!1,warning:"Channel ".concat(channel," should not be used with an unsorted discrete field.")};case "strokeDash":return contains(["ordinal","nominal"],fieldDef.type)?COMPATIBLE:{compatible:!1,warning:"StrokeDash channel should be used with only discrete data."};case "shape":return contains(["ordinal",
"nominal","geojson"],fieldDef.type)?COMPATIBLE:{compatible:!1,warning:"Shape channel should be used with only either discrete or geojson data."};case "order":return"nominal"!==fieldDef.type||"sort"in fieldDef?COMPATIBLE:{compatible:!1,warning:"Channel order is inappropriate for nominal field, which has no inherent order."}}}function isFieldOrDatumDefForTimeFormat(fieldOrDatumDef){var {formatType}=getFormatMixins(fieldOrDatumDef),JSCompiler_temp;if(!(JSCompiler_temp="time"===formatType)){if(formatType=
!formatType)formatType=fieldOrDatumDef&&("temporal"===fieldOrDatumDef.type||isFieldDef(fieldOrDatumDef)&&!!fieldOrDatumDef.timeUnit);JSCompiler_temp=formatType}return JSCompiler_temp}function valueExpr(v,$jscomp$destructuring$var31){var {timeUnit,type,wrapTime,undefinedIfExprNotRequired}=$jscomp$destructuring$var31;var _normalizeTimeUnit2=($jscomp$destructuring$var31=timeUnit&&(null===(_normalizeTimeUnit2=normalizeTimeUnit(timeUnit))||void 0===_normalizeTimeUnit2?void 0:_normalizeTimeUnit2.unit))||
"temporal"===type;let expr;isExprRef(v)?expr=v.expr:isSignalRef(v)?expr=v.signal:isDateTime(v)?(_normalizeTimeUnit2=!0,expr=dateTimeToExpr(v)):(isString(v)||isNumber(v))&&_normalizeTimeUnit2&&(expr="datetime(".concat(JSON.stringify(v),")"),LOCAL_SINGLE_TIMEUNIT_INDEX[$jscomp$destructuring$var31]&&(isNumber(v)&&1E4>v||isString(v)&&isNaN(Date.parse(v)))&&(expr=dateTimeToExpr({[$jscomp$destructuring$var31]:v})));return expr?wrapTime&&_normalizeTimeUnit2?"time(".concat(expr,")"):expr:undefinedIfExprNotRequired?
void 0:JSON.stringify(v)}function valueArray(fieldOrDatumDef,values){const {type}=fieldOrDatumDef;return values.map(v=>{const expr=valueExpr(v,{timeUnit:isFieldDef(fieldOrDatumDef)?fieldOrDatumDef.timeUnit:void 0,type,undefinedIfExprNotRequired:!0});return void 0!==expr?{signal:expr}:v})}function binRequiresRange(fieldDef,channel){return isBinning(fieldDef.bin)?!!SCALE_CHANNEL_INDEX[channel]&&contains(["ordinal","nominal"],fieldDef.type):(console.warn("Only call this method for binned field defs."),
!1)}function extractTitleConfig(titleConfig){var $jscomp$destructuring$var33=Object.assign({},titleConfig),anchor=titleConfig.anchor;const frame=titleConfig.frame,offset=titleConfig.offset,orient=titleConfig.orient;var color=titleConfig.color,subtitleColor=titleConfig.subtitleColor,subtitleFont=titleConfig.subtitleFont;const subtitleFontSize=titleConfig.subtitleFontSize,subtitleFontStyle=titleConfig.subtitleFontStyle,subtitleFontWeight=titleConfig.subtitleFontWeight,subtitleLineHeight=titleConfig.subtitleLineHeight;
titleConfig=titleConfig.subtitlePadding;$jscomp$destructuring$var33=(delete $jscomp$destructuring$var33.anchor,delete $jscomp$destructuring$var33.frame,delete $jscomp$destructuring$var33.offset,delete $jscomp$destructuring$var33.orient,delete $jscomp$destructuring$var33.color,delete $jscomp$destructuring$var33.subtitleColor,delete $jscomp$destructuring$var33.subtitleFont,delete $jscomp$destructuring$var33.subtitleFontSize,delete $jscomp$destructuring$var33.subtitleFontStyle,delete $jscomp$destructuring$var33.subtitleFontWeight,
delete $jscomp$destructuring$var33.subtitleLineHeight,delete $jscomp$destructuring$var33.subtitlePadding,$jscomp$destructuring$var33);color=Object.assign({},$jscomp$destructuring$var33,color?{fill:color}:{});anchor=Object.assign({},anchor?{anchor}:{},frame?{frame}:{},offset?{offset}:{},orient?{orient}:{});subtitleColor=Object.assign({},subtitleColor?{subtitleColor}:{},subtitleFont?{subtitleFont}:{},subtitleFontSize?{subtitleFontSize}:{},subtitleFontStyle?{subtitleFontStyle}:{},subtitleFontWeight?
{subtitleFontWeight}:{},subtitleLineHeight?{subtitleLineHeight}:{},titleConfig?{subtitlePadding:titleConfig}:{});subtitleFont=pick(color,["align","baseline","dx","dy","limit"]);return{titleMarkConfig:color,subtitleMarkConfig:subtitleFont,nonMark:anchor,subtitle:subtitleColor}}function isText(v){return isString(v)||isArray(v)&&isString(v[0])}function signalOrValueRefWithCondition(val){const condition=isArray(val.condition)?val.condition.map(conditionalSignalRefOrValue):conditionalSignalRefOrValue(val.condition);
return Object.assign({},signalRefOrValue(val),{condition})}function signalRefOrValue(value){if(isExprRef(value)){var $jscomp$destructuring$var35=Object.assign({},value);value=value.expr;$jscomp$destructuring$var35=(delete $jscomp$destructuring$var35.expr,$jscomp$destructuring$var35);return Object.assign({},{signal:value},$jscomp$destructuring$var35)}return value}function conditionalSignalRefOrValue(value){if(isExprRef(value)){var $jscomp$destructuring$var37=Object.assign({},value);value=value.expr;
$jscomp$destructuring$var37=(delete $jscomp$destructuring$var37.expr,$jscomp$destructuring$var37);return Object.assign({},{signal:value},$jscomp$destructuring$var37)}return value}function signalOrValueRef(value){if(isExprRef(value)){var $jscomp$destructuring$var39=Object.assign({},value);value=value.expr;$jscomp$destructuring$var39=(delete $jscomp$destructuring$var39.expr,$jscomp$destructuring$var39);return Object.assign({},{signal:value},$jscomp$destructuring$var39)}return isSignalRef(value)?value:
void 0!==value?{value}:void 0}function exprFromValueOrSignalRef(ref){return isSignalRef(ref)?ref.signal:$(ref.value)}function signalOrStringValue(v){return isSignalRef(v)?v.signal:null==v?null:$(v)}function applyMarkConfig(e,model,propsList){for(const property of propsList)propsList=getMarkConfig(property,model.markDef,model.config),void 0!==propsList&&(e[property]=signalOrValueRef(propsList));return e}function getStyles(mark){var _mark$style;return[].concat(mark.type,null!==(_mark$style=mark.style)&&
...
lex();var JSCompiler_object_inline_value_4049=new ASTNode("ArrayExpression");JSCompiler_object_inline_value_4049.elements=type;return type=JSCompiler_object_inline_value_4049}if(match("{")){type=[];JSCompiler_object_inline_value_4049={};var JSCompiler_object_inline_literal_4050=String;index$jscomp$0=lookahead.start;for(expect("{");!match("}");){index$jscomp$0=lookahead.start;var JSCompiler_object_inline_value_4051=lookahead;if(3===JSCompiler_object_inline_value_4051.type){var JSCompiler_object_inline_literal_4052=
parseObjectPropertyKey();expect(":");JSCompiler_object_inline_value_4051=parseConditionalExpression();JSCompiler_object_inline_value_4051=finishProperty("init",JSCompiler_object_inline_literal_4052,JSCompiler_object_inline_value_4051)}else 2===JSCompiler_object_inline_value_4051.type||7===JSCompiler_object_inline_value_4051.type?(throwUnexpected(JSCompiler_object_inline_value_4051),JSCompiler_object_inline_value_4051=void 0):(JSCompiler_object_inline_literal_4052=parseObjectPropertyKey(),expect(":"),
JSCompiler_object_inline_value_4051=parseConditionalExpression(),JSCompiler_object_inline_value_4051=finishProperty("init",JSCompiler_object_inline_literal_4052,JSCompiler_object_inline_value_4051));JSCompiler_object_inline_literal_4052="Identifier"===JSCompiler_object_inline_value_4051.key.type?JSCompiler_object_inline_value_4051.key.name:JSCompiler_object_inline_literal_4050(JSCompiler_object_inline_value_4051.key.value);JSCompiler_object_inline_literal_4052="$"+JSCompiler_object_inline_literal_4052;
Object.prototype.hasOwnProperty.call(JSCompiler_object_inline_value_4049,JSCompiler_object_inline_literal_4052)?throwError({},"Duplicate data property in object literal not allowed in strict mode"):JSCompiler_object_inline_value_4049[JSCompiler_object_inline_literal_4052]=!0;type.push(JSCompiler_object_inline_value_4051);match("}")||expect(",")}expect("}");JSCompiler_object_inline_value_4049=new ASTNode("ObjectExpression");JSCompiler_object_inline_value_4049.properties=type;return type=JSCompiler_object_inline_value_4049}type=
lookahead.type;index$jscomp$0=lookahead.start;if(3===type||legalKeywords[lookahead.value])JSCompiler_object_inline_value_4049=finishIdentifier(lex().value);else if(8===type||6===type)lookahead.octal&&throwError(lookahead,"Octal literals are not allowed in strict mode."),JSCompiler_object_inline_value_4049=finishLiteral(lex());else{if(4===type)throw Error("Disabled.");if(1===type)type=lex(),type.value="true"===type.value,JSCompiler_object_inline_value_4049=finishLiteral(type);else if(5===type)type=
lex(),type.value=null,JSCompiler_object_inline_value_4049=finishLiteral(type);else if(match("/")||match("/\x3d")){var value;lookahead=null;skipComment();type=index$jscomp$0;JSCompiler_object_inline_value_4049=source$jscomp$0[index$jscomp$0];assert("/"===JSCompiler_object_inline_value_4049,"Regular expression literal must start with a slash");JSCompiler_object_inline_literal_4050=source$jscomp$0[index$jscomp$0++];for(JSCompiler_object_inline_literal_4052=JSCompiler_object_inline_value_4051=!1;index$jscomp$0<
length$jscomp$0;)if(JSCompiler_object_inline_value_4049=source$jscomp$0[index$jscomp$0++],JSCompiler_object_inline_literal_4050+=JSCompiler_object_inline_value_4049,"\\"===JSCompiler_object_inline_value_4049)JSCompiler_object_inline_value_4049=source$jscomp$0[index$jscomp$0++],isLineTerminator(JSCompiler_object_inline_value_4049.charCodeAt(0))&&throwError({},"Invalid regular expression: missing /"),JSCompiler_object_inline_literal_4050+=JSCompiler_object_inline_value_4049;else if(isLineTerminator(JSCompiler_object_inline_value_4049.charCodeAt(0)))throwError({},
"Invalid regular expression: missing /");else if(JSCompiler_object_inline_value_4051)"]"===JSCompiler_object_inline_value_4049&&(JSCompiler_object_inline_value_4051=!1);else if("/"===JSCompiler_object_inline_value_4049){JSCompiler_object_inline_literal_4052=!0;break}else"["===JSCompiler_object_inline_value_4049&&(JSCompiler_object_inline_value_4051=!0);JSCompiler_object_inline_literal_4052||throwError({},"Invalid regular expression: missing /");JSCompiler_object_inline_value_4049=JSCompiler_object_inline_literal_4050.substr(1,
JSCompiler_object_inline_literal_4050.length-2);for(value=JSCompiler_object_inline_literal_4052="";index$jscomp$0<length$jscomp$0;){JSCompiler_object_inline_value_4051=source$jscomp$0[index$jscomp$0];if(!isIdentifierPart(JSCompiler_object_inline_value_4051.charCodeAt(0)))break;++index$jscomp$0;"\\"===JSCompiler_object_inline_value_4051&&index$jscomp$0<length$jscomp$0?throwError({},"Unexpected token %0","ILLEGAL"):(value+=JSCompiler_object_inline_value_4051,JSCompiler_object_inline_literal_4052+=JSCompiler_object_inline_value_4051)}0<=
value.search(/[^gimuy]/g)&&throwError({},"Invalid regular expression",value);JSCompiler_object_inline_value_4051=value;value=testRegExp(JSCompiler_object_inline_value_4049,JSCompiler_object_inline_value_4051);JSCompiler_object_inline_value_4049=finishLiteral({literal:JSCompiler_object_inline_literal_4050+JSCompiler_object_inline_literal_4052,value,regex:{pattern:JSCompiler_object_inline_value_4049,flags:JSCompiler_object_inline_value_4051},start:type,end:index$jscomp$0});peek()}else throwUnexpected(lex())}return JSCompiler_object_inline_value_4049}
...
text:"always",tick:"always"};var $jscomp$destructuring$var11=Object.assign({},ALL_MARKS);const ALL_MARKS_EXCEPT_GEOSHAPE=(delete $jscomp$destructuring$var11.geoshape,$jscomp$destructuring$var11),main=function(_$jscomp$0,method){let level=_$jscomp$0||0;return{level(_){return arguments.length?(level=+_,this):level},error(){1<=level&&log(method||"error","ERROR",arguments);return this},warn(){2<=level&&log(method||"warn","WARN",arguments);return this},info(){3<=level&&log(method||"log","INFO",arguments);
return this},debug(){4<=level&&log(method||"log","DEBUG",arguments);return this}}}(2);let current=main;const MONTHS="january february march april may june july august september october november december".split(" "),SHORT_MONTHS=MONTHS.map(m=>m.substr(0,3)),DAYS="sunday monday tuesday wednesday thursday friday saturday".split(" "),SHORT_DAYS=DAYS.map(d=>d.substr(0,3)),LOCAL_SINGLE_TIMEUNIT_INDEX={year:1,quarter:1,month:1,week:1,day:1,dayofyear:1,date:1,hours:1,minutes:1,seconds:1,milliseconds:1},TIMEUNIT_PARTS=
keys$jscomp$0(LOCAL_SINGLE_TIMEUNIT_INDEX),VEGALITE_TIMEFORMAT={"year-month":"%b %Y ","year-month-date":"%b %d, %Y "},VG_MARK_CONFIGS=keys$jscomp$0({aria:1,description:1,ariaRole:1,ariaRoleDescription:1,blend:1,opacity:1,fill:1,fillOpacity:1,stroke:1,strokeCap:1,strokeWidth:1,strokeOpacity:1,strokeDash:1,strokeDashOffset:1,strokeJoin:1,strokeOffset:1,strokeMiterLimit:1,startAngle:1,endAngle:1,padAngle:1,innerRadius:1,outerRadius:1,size:1,shape:1,interpolate:1,tension:1,orient:1,align:1,baseline:1,
text:1,dir:1,dx:1,dy:1,ellipsis:1,limit:1,radius:1,theta:1,angle:1,font:1,fontSize:1,fontWeight:1,fontStyle:1,lineBreak:1,lineHeight:1,cursor:1,href:1,tooltip:1,cornerRadius:1,cornerRadiusTopLeft:1,cornerRadiusTopRight:1,cornerRadiusBottomLeft:1,cornerRadiusBottomRight:1,aspect:1,width:1,height:1,url:1,smooth:1}),VG_MARK_INDEX={arc:1,area:1,group:1,image:1,line:1,path:1,rect:1,rule:1,shape:1,symbol:1,text:1,trail:1},VG_CORNERRADIUS_CHANNELS=["cornerRadius","cornerRadiusTopLeft","cornerRadiusTopRight",
"cornerRadiusBottomLeft","cornerRadiusBottomRight"],ScaleType={LINEAR:"linear",LOG:"log",POW:"pow",SQRT:"sqrt",SYMLOG:"symlog",IDENTITY:"identity",SEQUENTIAL:"sequential",TIME:"time",UTC:"utc",QUANTILE:"quantile",QUANTIZE:"quantize",THRESHOLD:"threshold",BIN_ORDINAL:"bin-ordinal",ORDINAL:"ordinal",POINT:"point",BAND:"band"},SCALE_CATEGORY_INDEX={linear:"numeric",log:"numeric",pow:"numeric",sqrt:"numeric",symlog:"numeric",identity:"numeric",sequential:"numeric",time:"time",utc:"time",ordinal:"ordinal",
...
typeWithExplicit){super({},{name});_defineProperty(this,"merged",!1);this.setWithExplicit("type",typeWithExplicit)}domainDefinitelyIncludesZero(){return!1!==this.get("zero")?!0:some(this.get("domains"),d=>isArray(d)&&2===d.length&&0>=d[0]&&0<=d[1])}}const RANGE_PROPERTIES=["range","scheme"],scaleRules={bins:$jscomp$destructuring$var178=>{var {model,fieldOrDatumDef}=$jscomp$destructuring$var178;return isFieldDef(fieldOrDatumDef)?bins$jscomp$1(model,fieldOrDatumDef):void 0},interpolate:$jscomp$destructuring$var179=>
{var {channel,fieldOrDatumDef}=$jscomp$destructuring$var179;$jscomp$destructuring$var179=fieldOrDatumDef.type;$jscomp$destructuring$var179=contains(["color","fill","stroke"],channel)&&"nominal"!==$jscomp$destructuring$var179?"hcl":void 0;return $jscomp$destructuring$var179},nice:$jscomp$destructuring$var180=>{var {scaleType,channel,fieldOrDatumDef}=$jscomp$destructuring$var180;var _getFieldDef;$jscomp$destructuring$var180=(null===(_getFieldDef=getFieldDef(fieldOrDatumDef))||void 0===_getFieldDef?
0:_getFieldDef.bin)||contains([ScaleType.TIME,ScaleType.UTC],scaleType)?void 0:channel in POSITION_SCALE_CHANNEL_INDEX?!0:void 0;return $jscomp$destructuring$var180},padding:$jscomp$destructuring$var181=>{var {channel,scaleType,fieldOrDatumDef,markDef,config}=$jscomp$destructuring$var181;a:{$jscomp$destructuring$var181=config.scale;var barConfig=config.bar;if(channel in POSITION_SCALE_CHANNEL_INDEX){if(scaleType in CONTINUOUS_TO_CONTINUOUS_INDEX){if(void 0!==$jscomp$destructuring$var181.continuousPadding){$jscomp$destructuring$var181=
$jscomp$destructuring$var181.continuousPadding;break a}const {type,orient}=markDef;if("bar"===type&&(!isFieldDef(fieldOrDatumDef)||!fieldOrDatumDef.bin&&!fieldOrDatumDef.timeUnit)&&("vertical"===orient&&"x"===channel||"horizontal"===orient&&"y"===channel)){$jscomp$destructuring$var181=barConfig.continuousBandSize;break a}}if(scaleType===ScaleType.POINT){$jscomp$destructuring$var181=$jscomp$destructuring$var181.pointPadding;break a}}$jscomp$destructuring$var181=void 0}return $jscomp$destructuring$var181},
paddingInner:$jscomp$destructuring$var182=>{var {scalePadding,channel,markDef,config}=$jscomp$destructuring$var182;if(void 0!==scalePadding)$jscomp$destructuring$var182=void 0;else if(channel in POSITION_SCALE_CHANNEL_INDEX){const {bandPaddingInner,barBandPaddingInner,rectBandPaddingInner}=config.scale;$jscomp$destructuring$var182=getFirstDefined(bandPaddingInner,"bar"===markDef.type?barBandPaddingInner:rectBandPaddingInner)}else $jscomp$destructuring$var182=void 0;return $jscomp$destructuring$var182},
paddingOuter:$jscomp$destructuring$var183=>{var {scalePadding,channel,scaleType,scalePaddingInner,config}=$jscomp$destructuring$var183;void 0!==scalePadding?$jscomp$destructuring$var183=void 0:channel in POSITION_SCALE_CHANNEL_INDEX&&scaleType===ScaleType.BAND?({bandPaddingOuter:$jscomp$destructuring$var183}=config.scale,$jscomp$destructuring$var183=getFirstDefined($jscomp$destructuring$var183,isSignalRef(scalePaddingInner)?{signal:"".concat(scalePaddingInner.signal,"/2")}:scalePaddingInner/2)):$jscomp$destructuring$var183=
void 0;return $jscomp$destructuring$var183},reverse:$jscomp$destructuring$var184=>{var {fieldOrDatumDef,scaleType,channel,config}=$jscomp$destructuring$var184;$jscomp$destructuring$var184=isFieldDef(fieldOrDatumDef)?fieldOrDatumDef.sort:void 0;var scaleConfig=config.scale;$jscomp$destructuring$var184="x"===channel&&void 0!==scaleConfig.xReverse?scaleType in CONTINUOUS_DOMAIN_INDEX&&"descending"===$jscomp$destructuring$var184?isSignalRef(scaleConfig.xReverse)?{signal:"!".concat(scaleConfig.xReverse.signal)}:
!scaleConfig.xReverse:scaleConfig.xReverse:scaleType in CONTINUOUS_DOMAIN_INDEX&&"descending"===$jscomp$destructuring$var184?!0:void 0;return $jscomp$destructuring$var184},zero:$jscomp$destructuring$var185=>{var {channel,fieldOrDatumDef,domain,markDef,scaleType}=$jscomp$destructuring$var185;a:if(domain&&"unaggregated"!==domain&&scaleType in CONTINUOUS_DOMAIN_INDEX){if(isArray(domain)&&($jscomp$destructuring$var185=domain[domain.length-1],0>=domain[0]&&0<=$jscomp$destructuring$var185)){$jscomp$destructuring$var185=
!0;break a}$jscomp$destructuring$var185=!1}else if("size"!==channel||"quantitative"!==fieldOrDatumDef.type||scaleType in CONTINUOUS_TO_DISCRETE_INDEX)if(isFieldDef(fieldOrDatumDef)&&fieldOrDatumDef.bin||!contains([...POSITION_SCALE_CHANNELS,...POLAR_POSITION_SCALE_CHANNELS],channel))$jscomp$destructuring$var185=!1;else{const {orient,type}=markDef;$jscomp$destructuring$var185=contains(["bar","area","line","trail"],type)&&("horizontal"===orient&&"y"===channel||"vertical"===orient&&"x"===channel)?!1:
!0}else $jscomp$destructuring$var185=!0;return $jscomp$destructuring$var185}},scaleTypeTieBreaker=tieBreakByComparing((st1,st2)=>SCALE_PRECEDENCE_INDEX[st1]-SCALE_PRECEDENCE_INDEX[st2]);class NameMap{constructor(){_defineProperty(this,"nameMap",void 0);this.nameMap={}}rename(oldName,newName){this.nameMap[oldName]=newName}has(name){return void 0!==this.nameMap[name]}get(name){for(;this.nameMap[name]&&name!==this.nameMap[name];)name=this.nameMap[name];return name}}class Model{constructor(spec,type,
...
void 0:titleBand}return Object.assign({},{padding:$jscomp$destructuring$var189},this.assembleDefaultLayout(),$jscomp$destructuring$var190,JSCompiler_inline_result?{titleBand:JSCompiler_inline_result}:{})}}assembleDefaultLayout(){return{}}assembleHeaderMarks(){const {layoutHeaders}=this.component;let headerMarks=[];for(const channel of FACET_CHANNELS)layoutHeaders[channel].title&&headerMarks.push(assembleTitleGroup(this,channel));for(const channel of HEADER_CHANNELS)headerMarks=headerMarks.concat(assembleHeaderGroups(this,
channel));return headerMarks}assembleAxes(){return assembleAxes(this.component.axes,this.config)}assembleLegends(){return assembleLegends(this)}assembleProjections(){var JSCompiler_inline_result=isLayerModel(this)||isConcatModel(this)?assembleProjectionsForModelAndChildren(this):assembleProjectionForModel(this);return JSCompiler_inline_result}assembleTitle(){var _this$title,$jscomp$destructuring$var191=null!==(_this$title=this.title)&&void 0!==_this$title?_this$title:{};_this$title=Object.assign({},
$jscomp$destructuring$var191);$jscomp$destructuring$var191=$jscomp$destructuring$var191.encoding;_this$title=(delete _this$title.encoding,_this$title);$jscomp$destructuring$var191=Object.assign({},extractTitleConfig(this.config.title).nonMark,_this$title,$jscomp$destructuring$var191?{encode:{update:$jscomp$destructuring$var191}}:{});if($jscomp$destructuring$var191.text){if(contains(["unit","layer"],this.type)){if(contains(["middle",void 0],$jscomp$destructuring$var191.anchor)){var _title$frame;$jscomp$destructuring$var191.frame=
null!==(_title$frame=$jscomp$destructuring$var191.frame)&&void 0!==_title$frame?_title$frame:"group"}}else{var _title$anchor;$jscomp$destructuring$var191.anchor=null!==(_title$anchor=$jscomp$destructuring$var191.anchor)&&void 0!==_title$anchor?_title$anchor:"start"}return isEmpty($jscomp$destructuring$var191)?void 0:$jscomp$destructuring$var191}}assembleGroup(signals){signals=void 0===signals?[]:signals;const group={};signals=signals.concat(this.assembleSignals());0<signals.length&&(group.signals=
...
this.parent?this.parent.getScaleComponent(channel):void 0}getSelectionComponent(variableName,origName){let sel=this.component.selection[variableName];!sel&&this.parent&&(sel=this.parent.getSelectionComponent(variableName,origName));if(!sel)throw Error('Cannot find a selection named "'.concat(origName,'".'));return sel}hasAxisOrientSignalRef(){var _this$component$axes$,_this$component$axes$2;return(null===(_this$component$axes$=this.component.axes.x)||void 0===_this$component$axes$?void 0:_this$component$axes$.some(a=>
a.hasOrientSignalRef()))||(null===(_this$component$axes$2=this.component.axes.y)||void 0===_this$component$axes$2?void 0:_this$component$axes$2.some(a=>a.hasOrientSignalRef()))}}class ModelWithField extends Model{vgField(channel,opt){opt=void 0===opt?{}:opt;if(channel=this.fieldDef(channel))return vgField(channel,opt)}reduceFieldDef(f,init){return reduce(this.getMapping(),(acc,cd,c)=>(cd=getFieldDef(cd))?f(acc,cd,c):acc,init)}forEachFieldDef(f,t){forEach(this.getMapping(),(cd,c)=>{(cd=getFieldDef(cd))&&
f(cd,c)},t)}}class DensityTransformNode extends DataFlowNode{clone(){return new DensityTransformNode(null,duplicate(this.transform))}constructor(parent,transform){var _this$transform$as,_specifiedAs$,_specifiedAs$2;super(parent);this.transform=transform;this.transform=duplicate(transform);parent=null!==(_this$transform$as=this.transform.as)&&void 0!==_this$transform$as?_this$transform$as:[void 0,void 0];this.transform.as=[null!==(_specifiedAs$=parent[0])&&void 0!==_specifiedAs$?_specifiedAs$:"value",
null!==(_specifiedAs$2=parent[1])&&void 0!==_specifiedAs$2?_specifiedAs$2:"density"]}dependentFields(){var _this$transform$group;return new Set([this.transform.density,...null!==(_this$transform$group=this.transform.groupby)&&void 0!==_this$transform$group?_this$transform$group:[]])}producedFields(){return new Set(this.transform.as)}hash(){return"DensityTransform ".concat(hash(this.transform))}assemble(){var $jscomp$destructuring$var193=this.transform,$jscomp$destructuring$var194=Object.assign({},
$jscomp$destructuring$var193);$jscomp$destructuring$var193=$jscomp$destructuring$var193.density;$jscomp$destructuring$var194=(delete $jscomp$destructuring$var194.density,$jscomp$destructuring$var194);return Object.assign({},{type:"kde",field:$jscomp$destructuring$var193},$jscomp$destructuring$var194)}}class FilterInvalidNode extends DataFlowNode{clone(){return new FilterInvalidNode(null,Object.assign({},this.filter))}constructor(parent,filter){super(parent);this.filter=filter}static make(parent,model){const {config,
mark,markDef}=model;if("filter"!==getMarkPropOrConfig("invalid",markDef,config))return null;const filter=model.reduceFieldDef((aggregator,fieldDef,channel)=>{(channel=!!SCALE_CHANNEL_INDEX[channel]&&model.getScaleComponent(channel))&&channel.get("type")in CONTINUOUS_DOMAIN_INDEX&&"count"!==fieldDef.aggregate&&!isPathMark(mark)&&(aggregator[fieldDef.field]=fieldDef);return aggregator},{});return keys$jscomp$0(filter).length?new FilterInvalidNode(parent,filter):null}dependentFields(){return new Set(keys$jscomp$0(this.filter))}producedFields(){return new Set}hash(){return"FilterInvalid ".concat(hash(this.filter))}assemble(){const filters=
...
" ").concat(hash(this.fields)," ").concat(hash(this.as))}assemble(){return{type:"geopoint",projection:this.projection,fields:this.fields,as:this.as}}}class ImputeNode extends DataFlowNode{clone(){return new ImputeNode(null,duplicate(this.transform))}constructor(parent,transform){super(parent);this.transform=transform}dependentFields(){var _this$transform$group;return new Set([this.transform.impute,this.transform.key,...null!==(_this$transform$group=this.transform.groupby)&&void 0!==_this$transform$group?
...
xDef.impute?xDef:yDef.impute?yDef:void 0;if(void 0===imputedChannel)return;xDef=xDef.impute?yDef:yDef.impute?xDef:void 0;const {method,value,frame,keyvals}=imputedChannel.impute;model=pathGroupingFields(model.mark,encoding);return new ImputeNode(parent,Object.assign({},{impute:imputedChannel.field,key:xDef.field},method?{method}:{},void 0!==value?{value}:{},frame?{frame}:{},void 0!==keyvals?{keyvals}:{},model.length?{groupby:model}:{}))}return null}hash(){return"Impute ".concat(hash(this.transform))}assemble(){const {impute,
