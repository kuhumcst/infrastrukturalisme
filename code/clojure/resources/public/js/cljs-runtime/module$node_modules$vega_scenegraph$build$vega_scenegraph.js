...
{id:item});vegaUtil.isFunction(clip)?renderer.path=clip(null):hasCornerRadius(size)?renderer.path=rectangle(null,size,0,0):(renderer.width=size.width||0,renderer.height=size.height||0);return"url(#"+item+")"}function Bounds(b){this.clear();b&&this.union(b)}function Item(mark){this.mark=mark;this.bounds=this.bounds||new Bounds}function GroupItem(mark){Item.call(this,mark);this.items=this.items||[]}function ResourceLoader(customLoader){this._pending=0;this._loader=customLoader||vegaLoader.loader()}
function boundStroke(bounds,item,miter){if(item.stroke&&0!==item.opacity&&0!==item.strokeOpacity){const sw=null!=item.strokeWidth?+item.strokeWidth:1;bounds.expand(sw+(miter?item.strokeJoin&&"miter"!==item.strokeJoin?0:sw:0))}return bounds}function context$jscomp$0(_){bounds=_;return context$jscomp$0}function noop(){}function addL(x,y){x=lx=x;y=ly=y;bounds.add(x,y)}function addX(x){bounds.add(x,bounds.y1)}function addY(y){bounds.add(bounds.x1,y)}function quadExtrema(x0,x1,x2,cb){x2=(x0-x1)/(x0+x2-
2*x1);0<x2&&1>x2&&cb(x0+(x1-x0)*x2)}function cubicExtrema(x0,x1,x2,x3,cb){const a=x3-x0+3*x1-3*x2,b=x0+x2-2*x1;var c=x0-x1;let t0=0,t1=0;1E-14<Math.abs(a)?(c=b*b+c*a,0<=c&&(c=Math.sqrt(c),t0=(-b+c)/a,t1=(-b-c)/a)):t0=.5*c/b;0<t0&&1>t0&&cb(cubic(t0,x0,x1,x2,x3));0<t1&&1>t1&&cb(cubic(t1,x0,x1,x2,x3))}function cubic(t,x0,x1,x2,x3){const s=1-t,s2=s*s,t2=t*t;return s2*s*x0+3*s2*t*x1+3*s*t2*x2+t2*t*x3}function intersectPath(draw){return function(item,brush){if(!context$1)return!0;draw(context$1,item);b$jscomp$0.clear().union(item.bounds).intersect(brush).round();
const {x1,y1,x2,y2}=b$jscomp$0;for(item=y1;item<=y2;++item)for(brush=x1;brush<=x2;++brush)if(context$1.isPointInPath(brush,item))return!0;return!1}}function intersectPoint(item,box){return box.contains(item.x||0,item.y||0)}function intersectRect(item,box){const x=item.x||0,y=item.y||0;return box.intersects(b$jscomp$0.set(x,y,x+(item.width||0),y+(item.height||0)))}function intersectRule(item,box){const x=item.x||0,y=item.y||0;return intersectBoxLine(box,x,y,null!=item.x2?item.x2:x,null!=item.y2?item.y2:
y)}function intersectBoxLine(box,x,y,u,v){const {x1,y1,x2,y2}=box;box=u-x;v-=y;u=0;let t1=1,p,q,r,e;for(e=0;4>e;++e){0===e&&(p=-box,q=-(x1-x));1===e&&(p=box,q=x2-x);2===e&&(p=-v,q=-(y1-y));3===e&&(p=v,q=y2-y);if(1E-10>Math.abs(p)&&0>q)return!1;r=q/p;if(0>p){if(r>t1)return!1;r>u&&(u=r)}else if(0<p){if(r<u)return!1;r<t1&&(t1=r)}}return!0}function blend(context,item){context.globalCompositeOperation=item.blend||"source-over"}function value$1(value,dflt){return null==value?dflt:value}function addStops(gradient,
stops){const n=stops.length;for(let i=0;i<n;++i)gradient.addColorStop(stops[i].offset,stops[i].color);return gradient}function gradient(context,spec,bounds){const w=bounds.width(),h=bounds.height();if("radial"===spec.gradient)context=context.createRadialGradient(bounds.x1+value$1(spec.x1,.5)*w,bounds.y1+value$1(spec.y1,.5)*h,Math.max(w,h)*value$1(spec.r1,0),bounds.x1+value$1(spec.x2,.5)*w,bounds.y1+value$1(spec.y2,.5)*h,Math.max(w,h)*value$1(spec.r2,.5));else{const x1=value$1(spec.x1,0),y1=value$1(spec.y1,
0),x2=value$1(spec.x2,1),y2=value$1(spec.y2,0);if(x1===x2||y1===y2||w===h)context=context.createLinearGradient(bounds.x1+x1*w,bounds.y1+y1*h,bounds.x1+x2*w,bounds.y1+y2*h);else{bounds=vegaCanvas.canvas(Math.ceil(w),Math.ceil(h));const ictx=bounds.getContext("2d");ictx.scale(w,h);ictx.fillStyle=addStops(ictx.createLinearGradient(x1,y1,x2,y2),spec.stops);ictx.fillRect(0,0,w,h);return context.createPattern(bounds,"no-repeat")}}return addStops(context,spec.stops)}function fill$jscomp$0(context,item,opacity){opacity*=
null==item.fillOpacity?1:item.fillOpacity;return 0<opacity?(context.globalAlpha=opacity,item=(opacity=item.fill)&&opacity.gradient?gradient(context,opacity,item.bounds):opacity,context.fillStyle=item,!0):!1}function stroke(context,item,opacity){var lw=null!=(lw=item.strokeWidth)?lw:1;if(0>=lw)return!1;opacity*=null==item.strokeOpacity?1:item.strokeOpacity;return 0<opacity?(context.globalAlpha=opacity,opacity=(opacity=item.stroke)&&opacity.gradient?gradient(context,opacity,item.bounds):opacity,context.strokeStyle=
opacity,context.lineWidth=lw,context.lineCap=item.strokeCap||"butt",context.lineJoin=item.strokeJoin||"miter",context.miterLimit=item.strokeMiterLimit||10,context.setLineDash&&(context.setLineDash(item.strokeDash||Empty),context.lineDashOffset=item.strokeDashOffset||0),!0):!1}function compare(a,b){return a.zindex-b.zindex||a.index-b.index}function zorder(scene){if(!scene.zdirty)return scene.zitems;var items=scene.items,output=[],n;var i=0;for(n=items.length;i<n;++i){var item=items[i];item.index=i;
item.zindex&&output.push(item)}scene.zdirty=!1;return scene.zitems=output.sort(compare)}function visit(scene,visitor){var items=scene.items,n;if(items&&items.length){var zitems=zorder(scene);if(zitems&&zitems.length){scene=0;for(n=items.length;scene<n;++scene)items[scene].zindex||visitor(items[scene]);items=zitems}scene=0;for(n=items.length;scene<n;++scene)visitor(items[scene])}}function pickVisit(scene,visitor){var items=scene.items,hit,i;if(!items||!items.length)return null;var zitems=zorder(scene);
zitems&&zitems.length&&(items=zitems);for(i=items.length;0<=--i;)if(hit=visitor(items[i]))return hit;if(items===zitems)for(items=scene.items,i=items.length;0<=--i;)if(!items[i].zindex&&(hit=visitor(items[i])))return hit;return null}function drawAll(path){return function(context,scene,bounds){visit(scene,function(item){bounds&&!bounds.intersects(item.bounds)||drawPath(path,context,item,item)})}}function drawOne(path){return function(context,scene,bounds){!scene.items.length||bounds&&!bounds.intersects(scene.bounds)||
drawPath(path,context,scene.items[0],scene.items)}}function drawPath(path,context,item,items){var opacity=null==item.opacity?1:item.opacity;0===opacity||path(context,items)||(blend(context,item),item.fill&&fill$jscomp$0(context,item,opacity)&&context.fill(),item.stroke&&stroke(context,item,opacity)&&context.stroke())}function pick(test){test=test||vegaUtil.truthy;return function(context,scene,x,y,gx,gy){x*=context.pixelRatio;y*=context.pixelRatio;return pickVisit(scene,function(item){var b=item.bounds;
if((!b||b.contains(gx,gy))&&b&&test(context,item,x,y,gx,gy))return item})}}function hitPath(path,filled){return function(context,o,x,y){var item=Array.isArray(o)?o[0]:o,fill=null==filled?item.fill:filled,stroke=item.stroke&&context.isPointInStroke;if(stroke){var lw=item.strokeWidth;item=item.strokeCap;context.lineWidth=null!=lw?lw:1;context.lineCap=null!=item?item:"butt"}return path(context,o)?!1:fill&&context.isPointInPath(x,y)||stroke&&context.isPointInStroke(x,y)}}function translate(x,y){return"translate("+
x+","+y+")"}function transformItem(item){return translate(item.x||0,item.y||0)+(item.angle?" rotate("+(item.angle+")"):"")+(item.scaleX||item.scaleY?" scale("+((item.scaleX||1)+","+(item.scaleY||1)+")"):"")}function markItemPath(type,shape,isect){function draw(context,item){var x=item.x||0,y=item.y||0,a=item.angle||0;context.translate(x,y);a&&context.rotate(a*=DegToRad);context.beginPath();shape(context,item);a&&context.rotate(-a);context.translate(-x,-y)}return{type,tag:"path",nested:!1,attr:function(emit,
item){emit("transform",transformItem(item));emit("d",shape(null,item))},bound:function(bounds,item){var x=item.x||0,y=item.y||0;shape(context$jscomp$0(bounds),item);boundStroke(bounds,item).translate(x,y);item.angle&&bounds.rotate(item.angle*DegToRad,x,y);return bounds},draw:drawAll(draw),pick:pick(hitPath(draw)),isect:isect||intersectPath(draw)}}function markMultiItemPath(type,shape,tip){function draw(context,items){context.beginPath();shape(context,items)}var hit=hitPath(draw);return{type,tag:"path",
nested:!0,attr:function(emit,item){item=item.mark.items;item.length&&emit("d",shape(null,item))},bound:function(bounds,mark){mark=mark.items;if(0===mark.length)return bounds;shape(context$jscomp$0(bounds),mark);return boundStroke(bounds,mark[0])},draw:drawOne(draw),pick:function(context,scene,x,y,gx,gy){var items=scene.items;scene=scene.bounds;if(!items||!items.length||scene&&!scene.contains(gx,gy))return null;x*=context.pixelRatio;y*=context.pixelRatio;return hit(context,items,x,y)?items[0]:null},
isect:intersectPoint,tip}}function clipGroup(context,group){context.beginPath();hasCornerRadius(group)?rectangle(context,group,0,0):context.rect(0,0,group.width||0,group.height||0);context.clip()}function offset$jscomp$0(item){const sw=value$1(item.strokeWidth,1);return null!=item.strokeOffset?item.strokeOffset:item.stroke&&.5<sw&&1.5>sw?.5-Math.abs(sw-1):0}function rectanglePath(context,group,x,y){const off=offset$jscomp$0(group);context.beginPath();rectangle(context,group,(x||0)+off,(y||0)+off)}
function getImage(item,renderer){var image$jscomp$0=item.image;if(!image$jscomp$0||item.url&&item.url!==image$jscomp$0.url)image$jscomp$0={complete:!1,width:0,height:0},renderer.loadImage(item.url).then(image=>{item.image=image;item.image.url=item.url});return image$jscomp$0}function imageWidth(item,image){return null!=item.width?item.width:image&&image.width?!1!==item.aspect&&item.height?item.height*image.width/image.height:image.width:0}function imageHeight(item,image){return null!=item.height?
item.height:image&&image.height?!1!==item.aspect&&item.width?item.width*image.height/image.width:image.height:0}function imageXOffset(align,w){return"center"===align?w/2:"right"===align?w:0}function imageYOffset(baseline,h){return"middle"===baseline?h/2:"bottom"===baseline?h:0}function path$jscomp$0(context,item){var path=item.path;if(null==path)return!0;var x=item.x||0,y=item.y||0,sx=item.scaleX||1,sy=item.scaleY||1,a=(item.angle||0)*DegToRad,cache=item.pathCache;cache&&cache.path===path||((item.pathCache=
cache=pathParse(path)).path=path);a&&context.rotate&&context.translate?(context.translate(x,y),context.rotate(a),pathRender(context,cache,0,0,sx,sy),context.rotate(-a),context.translate(-x,-y)):pathRender(context,cache,x,y,sx,sy)}function draw$2(context,item){context.beginPath();rectangle(context,item)}function path$2(context,item,opacity){if(item.stroke&&stroke(context,item,opacity)){opacity=item.x||0;var y1=item.y||0;var x2=null!=item.x2?item.x2:opacity;item=null!=item.y2?item.y2:y1;context.beginPath();
context.moveTo(opacity,y1);context.lineTo(x2,item);return!0}return!1}function useCanvas(use){textMetrics.width=use&&context$1?measureWidth:estimateWidth}function estimateWidth(item,text){text=textValue(item,text);item=fontSize(item);return~~(.8*text.length*item)}function measureWidth(item,text){return 0>=fontSize(item)||!(text=textValue(item,text))?0:_measureWidth(text,font(item))}function _measureWidth(text,currentFont){const key=`(${currentFont}) ${text}`;let width=widthCache.get(key);void 0===
width&&(context$1.font=currentFont,width=context$1.measureText(text).width,widthCache.set(key,width));return width}function fontSize(item){return null!=item.fontSize?+item.fontSize||0:11}function lineHeight(item){return null!=item.lineHeight?item.lineHeight:fontSize(item)+2}function textLines(item){item=item.lineBreak&&item.text&&!vegaUtil.isArray(item.text)?item.text.split(item.lineBreak):item.text;return vegaUtil.isArray(item)?1<item.length?item:item[0]:item}function textValue(item,line){line=null==
line?"":(line+"").trim();if(0<item.limit&&line.length){var limit=+item.limit,width=widthGetter(item);if(!(width(line)<limit)){var ellipsis=item.ellipsis||"…",rtl="rtl"===item.dir;item=0;var hi=line.length;limit-=width(ellipsis);if(rtl){for(;item<hi;)rtl=item+hi>>>1,width(line.slice(rtl))>limit?item=rtl+1:hi=rtl;line=ellipsis+line.slice(item)}else{for(;item<hi;)rtl=1+(item+hi>>>1),width(line.slice(0,rtl))<limit?item=rtl:hi=rtl-1;line=line.slice(0,item)+ellipsis}}}return line}function widthGetter(item){if(textMetrics.width===
measureWidth){const currentFont=font(item);return text=>_measureWidth(text,currentFont)}{const currentFontHeight=fontSize(item);return text=>~~(.8*text.length*currentFontHeight)}}function fontFamily(item,quote){item=item.font;return(quote&&item?String(item).replace(/"/g,"'"):item)||"sans-serif"}function font(item,quote){return(item.fontStyle?item.fontStyle+" ":"")+(item.fontVariant?item.fontVariant+" ":"")+(item.fontWeight?item.fontWeight+" ":"")+fontSize(item)+"px "+fontFamily(item,quote)}function offset$1(item){var baseline=
item.baseline,h=fontSize(item);return Math.round("top"===baseline?.79*h:"middle"===baseline?.3*h:"bottom"===baseline?-.21*h:"line-top"===baseline?.29*h+.5*lineHeight(item):"line-bottom"===baseline?.29*h-.5*lineHeight(item):0)}function anchorPoint(item){var x=item.x||0,y=item.y||0,r=item.radius||0;r&&(item=(item.theta||0)-HalfPi,x+=r*Math.cos(item),y+=r*Math.sin(item));tempBounds.x1=x;tempBounds.y1=y;return tempBounds}function bound$5(bounds,item,mode){var h=textMetrics.height(item),a=item.align,p=
anchorPoint(item),x=p.x1;p=p.y1;var dx=item.dx||0,dy=(item.dy||0)+offset$1(item)-Math.round(.8*h),tl=textLines(item);vegaUtil.isArray(tl)?(h+=lineHeight(item)*(tl.length-1),tl=tl.reduce((w,t)=>Math.max(w,textMetrics.width(item,t)),0)):tl=textMetrics.width(item,tl);"center"===a?dx-=tl/2:"right"===a&&(dx-=tl);bounds.set(dx+=x,dy+=p,dx+tl,dy+h);if(item.angle&&!mode)bounds.rotate(item.angle*DegToRad,x,p);else if(2===mode)return bounds.rotatedPoints(item.angle*DegToRad,x,p);return bounds}function boundItem(item,
func,opt){var type=Marks[item.mark.marktype];func=func||type.bound;type.nested&&(item=item.mark);return func(item.bounds||(item.bounds=new Bounds),item,opt)}function boundMark(mark,bounds,opt){var type=Marks[mark.marktype],bound=type.bound,items=mark.items,hasItems=items&&items.length;if(type.nested)return hasItems?mark=items[0]:(DUMMY.mark=mark,mark=DUMMY),opt=boundItem(mark,bound,opt),bounds=bounds&&bounds.union(opt)||opt;bounds=bounds||mark.bounds&&mark.bounds.clear()||new Bounds;if(hasItems)for(type=
0,hasItems=items.length;type<hasItems;++type)bounds.union(boundItem(items[type],bound,opt));return mark.bounds=bounds}function sceneToJSON(scene,indent){return JSON.stringify(scene,keys,indent)}function sceneFromJSON(json){json="string"===typeof json?JSON.parse(json):json;return initialize(json)}function initialize(scene){var type=scene.marktype,items=scene.items,n;if(items){var i=0;for(n=items.length;i<n;++i){var parent=type?"mark":"group";items[i][parent]=scene;items[i].zindex&&(items[i][parent].zdirty=
!0);"group"===(type||parent)&&initialize(items[i])}}type&&boundMark(scene);return scene}function Scenegraph(scene){arguments.length?this.root=sceneFromJSON(scene):(this.root=createMark({marktype:"group",name:"root",role:"frame"}),this.root.items=[new GroupItem(this.root)])}function createMark(def,group){group={bounds:new Bounds,clip:!!def.clip,group,interactive:!1===def.interactive?!1:!0,items:[],marktype:def.marktype,name:def.name||void 0,role:def.role||void 0,zindex:def.zindex||0};null!=def.aria&&
(group.aria=def.aria);def.description&&(group.description=def.description);return group}function domCreate(doc,tag,ns){!doc&&"undefined"!==typeof document&&document.createElement&&(doc=document);return doc?ns?doc.createElementNS(ns,tag):doc.createElement(tag):null}function domFind(el,tag){tag=tag.toLowerCase();el=el.childNodes;for(var i=0,n=el.length;i<n;++i)if(el[i].tagName.toLowerCase()===tag)return el[i]}function domChild(el,index,tag,ns){index=el.childNodes[index];if(!index||index.tagName.toLowerCase()!==
...
h._tooltipHandler=listener(h,(evt,item)=>{h.handleTooltip(evt,item,"mouseout"!==evt.type)})}function ariaItemAttributes(emit,item){var hide=!1===item.aria;emit("aria-hidden",hide||void 0);if(hide||null==item.description)for(const prop in AriaEncode)emit(AriaEncode[prop],void 0);else hide=item.mark.marktype,emit("aria-label",item.description),emit("role",item.ariaRole||("group"===hide?"graphics-object":"graphics-symbol")),emit("aria-roledescription",item.ariaRoleDescription||`${hide} mark`)}function ariaMarkAttributes(mark){return!1===
mark.aria?{["aria-hidden"]:!0}:AriaIgnore[mark.role]?null:AriaGuides[mark.role]?ariaGuide(mark,AriaGuides[mark.role]):ariaMark(mark)}function ariaMark(mark){const type=mark.marktype,recurse="group"===type||"text"===type||mark.items.some(_=>null!=_.description&&!1!==_.aria);return bundle(recurse?"graphics-object":"graphics-symbol",`${type} mark container`,mark.description)}function ariaGuide(mark,opt){try{const item=mark.items[0],caption=opt.caption||(()=>"");return bundle(opt.role||"graphics-symbol",
opt.desc,item.description||caption(item))}catch(err){return null}}function extractTitle(item){try{return vegaUtil.array(vegaUtil.peek(item.items).items[0].text).join(" ")}catch(err){return null}}function channelCaption(props){props=props.map(p=>p+("fill"===p||"stroke"===p?" color":""));return 2>props.length?props[0]:props.slice(0,-1).join(", ")+" and "+vegaUtil.peek(props)}function openTag(tag,attr,raw){tag="\x3c"+tag;var key;if(attr)for(key in attr){var val=attr[key];null!=val&&(tag+=" "+key+'\x3d"'+
(val+"").replace(/&/g,"\x26amp;").replace(/"/g,"\x26quot;")+'"')}raw&&(tag+=" "+raw);return tag+"\x3e"}function closeTag(tag){return"\x3c/"+tag+"\x3e"}function SVGRenderer(loader){Renderer.call(this,loader);this._dirtyID=0;this._dirty=[];this._defs=this._root=this._svg=null}function dirtyParents(item,id){for(;item&&item.dirty!==id;item=item.mark.group)if(item.dirty=id,item.mark&&item.mark.dirty!==id)item.mark.dirty=id;else break}function recurse$jscomp$0(renderer,el,group){el=el.lastChild.previousSibling;
let prev,idx=0;visit(group,item=>{prev=renderer.draw(el,item,prev);++idx});domClear(el,1+idx)}function bind(item,el,sibling,tag,svg){let node=item._svg;if(!node){var doc=el.ownerDocument;node=domCreate(doc,tag,ns);item._svg=node;item.mark&&(node.__data__=item,node.__values__={fill:"default"},"g"===tag&&(tag=domCreate(doc,"path",ns),node.appendChild(tag),tag.__data__=item,tag=domCreate(doc,"g",ns),node.appendChild(tag),tag.__data__=item,doc=domCreate(doc,"path",ns),node.appendChild(doc),doc.__data__=
item,doc.__values__={fill:"default"}))}(node.ownerSVGElement!==svg||node.parentNode&&1<node.parentNode.childNodes.length&&node.previousSibling!=sibling)&&el.insertBefore(node,sibling?sibling.nextSibling:el.firstChild);return node}function setStyle(el,name,value){value!==values[name]&&(null==value?el.style.removeProperty(name):el.style.setProperty(name,value+""),values[name]=value)}function emit(name,value,ns){if(value!==values[name]){if(ns){var el=element;null!=value?el.setAttributeNS(ns,name,value):
el.removeAttributeNS(ns,name)}else setAttribute(element,name,value);values[name]=value}}function setAttribute(el,name,value){null!=value?el.setAttribute(name,value):el.removeAttribute(name)}function href(){let loc;return"undefined"===typeof window?"":(loc=window.location).hash?loc.href.slice(0,-loc.hash.length):loc.href}function SVGStringRenderer(loader){Renderer.call(this,loader);this._text={head:"",bg:"",root:"",foot:"",defs:"",body:""};this._defs={gradient:{},clipping:{}}}function applyStyles(s,
item,scene,tag,defs){if(null==item)return s;"bgrect"===tag&&!1===scene.interactive&&(s["pointer-events"]="none");if("bgfore"===tag&&(!1===scene.interactive&&(s["pointer-events"]="none"),s.display="none",null!==item.fill))return s;"image"===tag&&!1===item.smooth&&(s.style="image-rendering: optimizeSpeed; image-rendering: pixelated;");"text"===tag&&(s["font-family"]=fontFamily(item),s["font-size"]=fontSize(item)+"px",item.fontStyle&&(s["font-style"]=item.fontStyle),item.fontVariant&&(s["font-variant"]=
item.fontVariant),item.fontWeight&&(s["font-weight"]=item.fontWeight));for(const prop in styles)scene=item[prop],tag=styles[prop],("transparent"!==scene||"fill"!==tag&&"stroke"!==tag)&&null!=scene&&(scene&&scene.gradient&&(scene=gradientRef(scene,defs.gradient,"")),s[tag]=scene);return s}function escape_text(s){return s.replace(/&/g,"\x26amp;").replace(/</g,"\x26lt;").replace(/>/g,"\x26gt;")}function intersectMark(mark,box,filter,hits){if(mark.bounds&&box.intersects(mark.bounds)&&("group"===mark.marktype||
!1!==mark.interactive&&(!filter||filter(mark)))){const items=mark.items;var type=mark.marktype;mark=items.length;let i=0;if("group"===type)for(;i<mark;++i)intersectGroup(items[i],box,filter,hits);else for(filter=Marks[type].isect;i<mark;++i)type=items[i],intersectItem(type,box,filter)&&hits.push(type)}return hits}function intersectGroup(group,box,filter,hits){filter&&filter(group.mark)&&intersectItem(group,box,Marks.group.isect)&&hits.push(group);const marks=group.items,n=marks&&marks.length;if(n){const x=
group.x||0;group=group.y||0;box.translate(-x,-group);for(let i=0;i<n;++i)intersectMark(marks[i],box,filter,hits);box.translate(x,group)}return hits}function intersectItem(item,box,test){const bounds=item.bounds;return box.encloses(bounds)||box.intersects(bounds)&&test(item,box)}function sceneEqual(a,b,key){return a===b?!0:"path"===key?pathEqual(a,b):a instanceof Date&&b instanceof Date?+a===+b:vegaUtil.isNumber(a)&&vegaUtil.isNumber(b)?1E-9>=Math.abs(a-b):a&&b&&(vegaUtil.isObject(a)||vegaUtil.isObject(b))?
null==a||null==b?!1:objectEqual(a,b):a==b}function pathEqual(a,b){return sceneEqual(pathParse(a),pathParse(b))}function objectEqual(a,b){var ka=Object.keys(a),kb=Object.keys(b),i;if(ka.length!==kb.length)return!1;ka.sort();kb.sort();for(i=ka.length-1;0<=i;i--)if(ka[i]!=kb[i])return!1;for(i=ka.length-1;0<=i;i--)if(kb=ka[i],!sceneEqual(a[kb],b[kb],kb))return!1;return typeof a===typeof b}var gradient_id=0,lookup={basis:{curve:d3Shape.curveBasis},"basis-closed":{curve:d3Shape.curveBasisClosed},"basis-open":{curve:d3Shape.curveBasisOpen},
bundle:{curve:d3Shape.curveBundle,tension:"beta",value:.85},cardinal:{curve:d3Shape.curveCardinal,tension:"tension",value:0},"cardinal-open":{curve:d3Shape.curveCardinalOpen,tension:"tension",value:0},"cardinal-closed":{curve:d3Shape.curveCardinalClosed,tension:"tension",value:0},"catmull-rom":{curve:d3Shape.curveCatmullRom,tension:"alpha",value:.5},"catmull-rom-closed":{curve:d3Shape.curveCatmullRomClosed,tension:"alpha",value:.5},"catmull-rom-open":{curve:d3Shape.curveCatmullRomOpen,tension:"alpha",
value:.5},linear:{curve:d3Shape.curveLinear},"linear-closed":{curve:d3Shape.curveLinearClosed},monotone:{horizontal:d3Shape.curveMonotoneY,vertical:d3Shape.curveMonotoneX},natural:{curve:d3Shape.curveNatural},step:{curve:d3Shape.curveStep},"step-after":{curve:d3Shape.curveStepAfter},"step-before":{curve:d3Shape.curveStepBefore}},cmdlen={m:2,l:2,h:1,v:1,c:6,s:4,q:4,t:2,a:7},regexp=[/([MLHVCSQTAZmlhvcsqtaz])/g,/###/,/(\d)([-+])/g,/\s|,|###/];const DegToRad=Math.PI/180,HalfPi=Math.PI/2,Tau=2*Math.PI,
HalfSqrt3=Math.sqrt(3)/2;var segmentCache={},bezierCache={},join=[].join,temp=["l",0,0,0,0,0,0,0],builtins={circle:{draw:function(context,size){size=Math.sqrt(size)/2;context.moveTo(size,0);context.arc(0,0,size,0,Tau)}},cross:{draw:function(context,size){size=Math.sqrt(size)/2;var s=size/2.5;context.moveTo(-size,-s);context.lineTo(-size,s);context.lineTo(-s,s);context.lineTo(-s,size);context.lineTo(s,size);context.lineTo(s,s);context.lineTo(size,s);context.lineTo(size,-s);context.lineTo(s,-s);context.lineTo(s,
-size);context.lineTo(-s,-size);context.lineTo(-s,-s);context.closePath()}},diamond:{draw:function(context,size){size=Math.sqrt(size)/2;context.moveTo(-size,0);context.lineTo(0,-size);context.lineTo(size,0);context.lineTo(0,size);context.closePath()}},square:{draw:function(context,size){size=Math.sqrt(size);var x=-size/2;context.rect(x,x,size,size)}},arrow:{draw:function(context,size){size=Math.sqrt(size)/2;var s=size/7,t=size/2.5,v=size/8;context.moveTo(-s,size);context.lineTo(s,size);context.lineTo(s,
-v);context.lineTo(t,-v);context.lineTo(0,-size);context.lineTo(-t,-v);context.lineTo(-s,-v);context.closePath()}},wedge:{draw:function(context,size){var r=Math.sqrt(size)/2;size=HalfSqrt3*r;var o=size-.5773502691896257*r;r/=4;context.moveTo(0,-size-o);context.lineTo(-r,size-o);context.lineTo(r,size-o);context.closePath()}},triangle:{draw:function(context,size){size=Math.sqrt(size)/2;var h=HalfSqrt3*size,o=h-.5773502691896257*size;context.moveTo(0,-h-o);context.lineTo(-size,h-o);context.lineTo(size,
h-o);context.closePath()}},"triangle-up":{draw:function(context,size){size=Math.sqrt(size)/2;var h=HalfSqrt3*size;context.moveTo(0,-h);context.lineTo(-size,h);context.lineTo(size,h);context.closePath()}},"triangle-down":{draw:function(context,size){size=Math.sqrt(size)/2;var h=HalfSqrt3*size;context.moveTo(0,h);context.lineTo(-size,-h);context.lineTo(size,-h);context.closePath()}},"triangle-right":{draw:function(context,size){size=Math.sqrt(size)/2;var h=HalfSqrt3*size;context.moveTo(h,0);context.lineTo(-h,
-size);context.lineTo(-h,size);context.closePath()}},"triangle-left":{draw:function(context,size){size=Math.sqrt(size)/2;var h=HalfSqrt3*size;context.moveTo(-h,0);context.lineTo(h,-size);context.lineTo(h,size);context.closePath()}},stroke:{draw:function(context,size){size=Math.sqrt(size)/2;context.moveTo(-size,0);context.lineTo(size,0)}}},custom={};const x$jscomp$0=item=>item.x||0,y$jscomp$0=item=>item.y||0,def$jscomp$0=item=>!1!==item.defined,arcShape=d3Shape.arc().startAngle(item=>item.startAngle||
0).endAngle(item=>item.endAngle||0).padAngle(item=>item.padAngle||0).innerRadius(item=>item.innerRadius||0).outerRadius(item=>item.outerRadius||0).cornerRadius(item=>item.cornerRadius||0),areavShape=d3Shape.area().x(x$jscomp$0).y1(y$jscomp$0).y0(item=>(item.y||0)+(item.height||0)).defined(def$jscomp$0),areahShape=d3Shape.area().y(y$jscomp$0).x1(x$jscomp$0).x0(item=>(item.x||0)+(item.width||0)).defined(def$jscomp$0),lineShape=d3Shape.line().x(x$jscomp$0).y(y$jscomp$0).defined(def$jscomp$0),rectShape=
vg_rect().x(x$jscomp$0).y(y$jscomp$0).width(item=>item.width||0).height(item=>item.height||0).cornerRadius(item=>value$jscomp$0(item.cornerRadiusTopLeft,item.cornerRadius)||0,item=>value$jscomp$0(item.cornerRadiusTopRight,item.cornerRadius)||0,item=>value$jscomp$0(item.cornerRadiusBottomRight,item.cornerRadius)||0,item=>value$jscomp$0(item.cornerRadiusBottomLeft,item.cornerRadius)||0),symbolShape=d3Shape.symbol().type(item=>symbols(item.shape||"circle")).size(item=>value$jscomp$0(item.size,64)),trailShape=
vg_trail().x(x$jscomp$0).y(y$jscomp$0).defined(def$jscomp$0).size(item=>item.size||1);var clip_id=1,prototype=Bounds.prototype;prototype.clone=function(){return new Bounds(this)};prototype.clear=function(){this.x1=+Number.MAX_VALUE;this.y1=+Number.MAX_VALUE;this.x2=-Number.MAX_VALUE;this.y2=-Number.MAX_VALUE;return this};prototype.empty=function(){return this.x1===+Number.MAX_VALUE&&this.y1===+Number.MAX_VALUE&&this.x2===-Number.MAX_VALUE&&this.y2===-Number.MAX_VALUE};prototype.equals=function(b){return this.x1===
b.x1&&this.y1===b.y1&&this.x2===b.x2&&this.y2===b.y2};prototype.set=function(x1,y1,x2,y2){x2<x1?(this.x2=x1,this.x1=x2):(this.x1=x1,this.x2=x2);y2<y1?(this.y2=y1,this.y1=y2):(this.y1=y1,this.y2=y2);return this};prototype.add=function(x,y){x<this.x1&&(this.x1=x);y<this.y1&&(this.y1=y);x>this.x2&&(this.x2=x);y>this.y2&&(this.y2=y);return this};prototype.expand=function(d){this.x1-=d;this.y1-=d;this.x2+=d;this.y2+=d;return this};prototype.round=function(){this.x1=Math.floor(this.x1);this.y1=Math.floor(this.y1);
this.x2=Math.ceil(this.x2);this.y2=Math.ceil(this.y2);return this};prototype.scale=function(s){this.x1*=s;this.y1*=s;this.x2*=s;this.y2*=s;return this};prototype.translate=function(dx,dy){this.x1+=dx;this.x2+=dx;this.y1+=dy;this.y2+=dy;return this};prototype.rotate=function(angle,x,y){angle=this.rotatedPoints(angle,x,y);return this.clear().add(angle[0],angle[1]).add(angle[2],angle[3]).add(angle[4],angle[5]).add(angle[6],angle[7])};prototype.rotatedPoints=function(angle,x,y){var {x1,y1,x2,y2}=this,
cos=Math.cos(angle);angle=Math.sin(angle);var cx=x-x*cos+y*angle;x=y-x*angle-y*cos;return[cos*x1-angle*y1+cx,angle*x1+cos*y1+x,cos*x1-angle*y2+cx,angle*x1+cos*y2+x,cos*x2-angle*y1+cx,angle*x2+cos*y1+x,cos*x2-angle*y2+cx,angle*x2+cos*y2+x]};prototype.union=function(b){b.x1<this.x1&&(this.x1=b.x1);b.y1<this.y1&&(this.y1=b.y1);b.x2>this.x2&&(this.x2=b.x2);b.y2>this.y2&&(this.y2=b.y2);return this};prototype.intersect=function(b){b.x1>this.x1&&(this.x1=b.x1);b.y1>this.y1&&(this.y1=b.y1);b.x2<this.x2&&
(this.x2=b.x2);b.y2<this.y2&&(this.y2=b.y2);return this};prototype.encloses=function(b){return b&&this.x1<=b.x1&&this.x2>=b.x2&&this.y1<=b.y1&&this.y2>=b.y2};prototype.alignsWith=function(b){return b&&(this.x1==b.x1||this.x2==b.x2||this.y1==b.y1||this.y2==b.y2)};prototype.intersects=function(b){return b&&!(this.x2<b.x1||this.x1>b.x2||this.y2<b.y1||this.y1>b.y2)};prototype.contains=function(x,y){return!(x<this.x1||x>this.x2||y<this.y1||y>this.y2)};prototype.width=function(){return this.x2-this.x1};
prototype.height=function(){return this.y2-this.y1};vegaUtil.inherits(GroupItem,Item);var prototype$1=ResourceLoader.prototype;prototype$1.pending=function(){return this._pending};prototype$1.sanitizeURL=function(uri){var loader=this;loader._pending+=1;return loader._loader.sanitize(uri,{context:"href"}).then(function(opt){--loader._pending;return opt}).catch(function(){--loader._pending;return null})};prototype$1.loadImage=function(uri){const loader=this,Image=vegaCanvas.image();loader._pending+=
1;return loader._loader.sanitize(uri,{context:"image"}).then(function(opt){const url=opt.href;if(!url||!Image)throw{url};const img=new Image;opt=vegaUtil.hasOwnProperty(opt,"crossOrigin")?opt.crossOrigin:"anonymous";null!=opt&&(img.crossOrigin=opt);img.onload=()=>{--loader._pending};img.onerror=()=>{--loader._pending};img.src=url;return img}).catch(function(e){--loader._pending;return{complete:!1,width:0,height:0,src:e&&e.url||""}})};prototype$1.ready=function(){var loader=this;return new Promise(function(accept){function poll(value){loader.pending()?
setTimeout(function(){poll(!0)},10):accept(value)}poll(!1)})};var bounds,lx,ly,circleThreshold=Tau-1E-8;context$jscomp$0.beginPath=noop;context$jscomp$0.closePath=noop;context$jscomp$0.moveTo=addL;context$jscomp$0.lineTo=addL;context$jscomp$0.rect=function(x,y,w,h){bounds.add(x+w,y+h);addL(x,y)};context$jscomp$0.quadraticCurveTo=function(x1,y1,x2,y2){quadExtrema(lx,x1,x2,addX);quadExtrema(ly,y1,y2,addY);addL(x2,y2)};context$jscomp$0.bezierCurveTo=function(x1,y1,x2,y2,x3,y3){cubicExtrema(lx,x1,x2,
x3,addX);cubicExtrema(ly,y1,y2,y3,addY);addL(x3,y3)};context$jscomp$0.arc=function(cx,cy,r,sa,ea,ccw){lx=r*Math.cos(ea)+cx;ly=r*Math.sin(ea)+cy;if(Math.abs(ea-sa)>circleThreshold)bounds.add(cx-r,cy-r),bounds.add(cx+r,cy+r);else{const update=a=>{bounds.add(r*Math.cos(a)+cx,r*Math.sin(a)+cy)};let s;update(sa);update(ea);if(ea!==sa)if(sa%=Tau,0>sa&&(sa+=Tau),ea%=Tau,0>ea&&(ea+=Tau),ea<sa&&(ccw=!ccw,s=sa,sa=ea,ea=s),ccw)for(ea-=Tau,s=sa-sa%HalfPi,sa=0;4>sa&&s>ea;++sa,s-=HalfPi)update(s);else for(s=sa-
sa%HalfPi+HalfPi,sa=0;4>sa&&s<ea;++sa,s+=HalfPi)update(s)}};var context$1=(context$1=vegaCanvas.canvas(1,1))?context$1.getContext("2d"):null;const b$jscomp$0=new Bounds;var Empty=[],arc$1=markItemPath("arc",function(context,item){return arcShape.context(context)(item)}),area$1=markMultiItemPath("area",function(context,items){var item=items[0];return("horizontal"===item.orient?areahShape:areavShape).curve(curves(item.interpolate||"linear",item.orient,item.tension)).context(context)(items)},function(a,
p){p="horizontal"===a[0].orient?p[1]:p[0];for(var z="horizontal"===a[0].orient?"y":"x",i=a.length,min=Infinity,hit,d;0<=--i;)!1!==a[i].defined&&(d=Math.abs(a[i][z]-p),d<min&&(min=d,hit=a[i]));return hit});const hitBackground=hitPath(rectanglePath),hitForeground=hitPath(rectanglePath,!1);var group$jscomp$0={type:"group",tag:"g",nested:!1,attr:function(emit,item){emit("transform",translate(item.x||0,item.y||0))},bound:function(bounds,group){if(!group.clip&&group.items){const items=group.items,m=items.length;
for(let j=0;j<m;++j)bounds.union(items[j].bounds)}(group.clip||group.width||group.height)&&!group.noBound&&bounds.add(0,0).add(group.width||0,group.height||0);boundStroke(bounds,group);return bounds.translate(group.x||0,group.y||0)},draw:function(context,scene,bounds){visit(scene,group=>{const gx=group.x||0,gy=group.y||0,fore=group.strokeForeground,opacity=null==group.opacity?1:group.opacity;(group.stroke||group.fill)&&opacity&&(rectanglePath(context,group,gx,gy),blend(context,group),group.fill&&
fill$jscomp$0(context,group,opacity)&&context.fill(),group.stroke&&!fore&&stroke(context,group,opacity)&&context.stroke());context.save();context.translate(gx,gy);group.clip&&clipGroup(context,group);bounds&&bounds.translate(-gx,-gy);visit(group,item=>{this.draw(context,item,bounds)});bounds&&bounds.translate(gx,gy);context.restore();fore&&group.stroke&&opacity&&(rectanglePath(context,group,gx,gy),blend(context,group),stroke(context,group,opacity)&&context.stroke())})},pick:function(context,scene,
x,y,gx,gy){if(scene.bounds&&!scene.bounds.contains(gx,gy)||!scene.items)return null;const cx=x*context.pixelRatio,cy=y*context.pixelRatio;return pickVisit(scene,group=>{let dx,dy;var hit=group.bounds;if(!hit||hit.contains(gx,gy)){dx=group.x||0;dy=group.y||0;hit=dx+(group.width||0);var fore=dy+(group.height||0);var ix=group.clip;if(!ix||!(gx<dx||gx>hit||gy<dy||gy>fore)){context.save();context.translate(dx,dy);dx=gx-dx;dy=gy-dy;if(ix&&hasCornerRadius(group)&&!hitBackground(context,group,cx,cy))return context.restore(),
null;fore=group.strokeForeground;if((ix=!1!==scene.interactive)&&fore&&group.stroke&&hitForeground(context,group,cx,cy))return context.restore(),group;hit=pickVisit(group,mark=>(!1!==mark.interactive||"group"===mark.marktype)&&mark.bounds&&mark.bounds.contains(dx,dy)?this.pick(mark,x,y,dx,dy):null);!hit&&ix&&(group.fill||!fore&&group.stroke)&&hitBackground(context,group,cx,cy)&&(hit=group);context.restore();return hit||null}}})},isect:intersectRect,content:function(emit,item,renderer){item=item.clip?
clip(renderer,item,item):null;emit("clip-path",item)},background:function(emit,item){emit("class","background");emit("aria-hidden",!0);{const off=offset$jscomp$0(item);emit("d",rectangle(null,item,off,off))}},foreground:function(emit,item){emit("class","foreground");emit("aria-hidden",!0);if(item.strokeForeground){{const off=offset$jscomp$0(item);emit("d",rectangle(null,item,off,off))}}else emit("d","")}},metadata={version:"1.1",xmlns:"http://www.w3.org/2000/svg","xmlns:xlink":"http://www.w3.org/1999/xlink"},
image$jscomp$1={type:"image",tag:"image",nested:!1,attr:function(emit,item,renderer){var img=getImage(item,renderer);renderer=imageWidth(item,img);const h=imageHeight(item,img),x=(item.x||0)-imageXOffset(item.align,renderer),y=(item.y||0)-imageYOffset(item.baseline,h);img=!img.src&&img.toDataURL?img.toDataURL():img.src||"";emit("href",img,metadata["xmlns:xlink"],"xlink:href");emit("transform",translate(x,y));emit("width",renderer);emit("height",h);emit("preserveAspectRatio",!1===item.aspect?"none":
"xMidYMid")},bound:function(bounds,item){var img=item.image;const w=imageWidth(item,img);img=imageHeight(item,img);const x=(item.x||0)-imageXOffset(item.align,w);item=(item.y||0)-imageYOffset(item.baseline,img);return bounds.set(x,item,x+w,item+img)},draw:function(context,scene,bounds){visit(scene,item=>{if(!bounds||bounds.intersects(item.bounds)){var img=getImage(item,this),w=imageWidth(item,img),h=imageHeight(item,img),x=(item.x||0)-imageXOffset(item.align,w),y=(item.y||0)-imageYOffset(item.baseline,
h),opacity;if(!1!==item.aspect){var ar0=img.width/img.height;var ar1=item.width/item.height;ar0===ar0&&ar1===ar1&&ar0!==ar1&&(ar1<ar0?(ar0=w/ar0,y+=(h-ar0)/2,h=ar0):(ar0*=h,x+=(w-ar0)/2,w=ar0))}if(img.complete||img.toDataURL)blend(context,item),context.globalAlpha=null!=(opacity=item.opacity)?opacity:1,context.imageSmoothingEnabled=!1!==item.smooth,context.drawImage(img,x,y,w,h)}})},pick:pick(),isect:vegaUtil.truthy,get:getImage,xOffset:imageXOffset,yOffset:imageYOffset},line$1=markMultiItemPath("line",
function(context,items){var item=items[0];return lineShape.curve(curves(item.interpolate||"linear",item.orient,item.tension)).context(context)(items)},function(a,p){for(var t=Math.pow(a[0].strokeWidth||1,2),i=a.length,dx,dy;0<=--i;)if(!1!==a[i].defined&&(dx=a[i].x-p[0],dy=a[i].y-p[1],dx=dx*dx+dy*dy,dx<t))return a[i];return null}),path$1={type:"path",tag:"path",nested:!1,attr:function(emit,item){var sy=item.scaleY||1;1===(item.scaleX||1)&&1===sy||emit("vector-effect","non-scaling-stroke");emit("transform",
transformItem(item));emit("d",item.path)},bound:function(bounds,item){path$jscomp$0(context$jscomp$0(bounds),item)?bounds.set(0,0,0,0):boundStroke(bounds,item,!0);item.angle&&bounds.rotate(item.angle*DegToRad,item.x||0,item.y||0);return bounds},draw:drawAll(path$jscomp$0),pick:pick(hitPath(path$jscomp$0)),isect:intersectPath(path$jscomp$0)},rect={type:"rect",tag:"path",nested:!1,attr:function(emit,item){emit("d",rectangle(null,item))},bound:function(bounds,item){var x,y;return boundStroke(bounds.set(x=
